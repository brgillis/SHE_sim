...
int main(int argc, char *argv[])
{
	...
	{
		printf(
				"USAGE type r0_bulge r0_disk z0_disk bulge_compression Npoints n_Sersic");
		exit(0);
	}
	...
	double *z, *z_new, *I_z, *F_z, *F_z_new, *z_inf, *I_z_inf, F_z_inf,
			*intensity;
	double I_z0 = 1.0;
	double redshift, d_L, r0bulge_kpc_i, r0disk_kpc_i, z0_kpc_i,
			squeeze_bulge_zfactor;
	...
	for (ir = 0; ir < rpoints; ir++)
	{
		...
		if (strncmp(galaxytype, "disk", 4) == 0)
		{
			I_r[ir] = I_disk_r0_i * exp(-radius[ir] / r0disk_kpc_i);
		} // r0_disk_pic_i is the SCALE LENGTH OF THE DISK (corresponds to h in Graham & Worley 2008)
		if (strncmp(galaxytype, "bulge", 5) == 0)
		{
					...
					* exp(
							-bn
									* (pow((radius[ir] / r0bulge_kpc_i),
											1 / nsersic) - 1));
		} // r0_bulge_kpc_i is the Effective radius/half light radius of the bulge (r_e in Graham & Worely 2008)
	...
	for (ir = 0; ir < rpoints_inf; ir++)
	{
		...
		if (strncmp(galaxytype, "disk", 4) == 0)
		{
			I_r_inf[ir] = I_disk_r0_i * exp(-radius_inf[ir] / r0disk_kpc_i) * 2
					* PI * radius_inf[ir];
		}
		if (strncmp(galaxytype, "bulge", 5) == 0)
		{
	...
	F_r_inf = cum_integral(radius_inf, 0.0, radius_inf[rpoints_inf - 1],
			I_r_inf, rpoints_inf);
		...
		for (ir = 0; ir < rpoints; ir++)
		{
			...
			if ((F_new >= F_r[ir]) && (F_new < F_r[ir + 1]))
			{
				...
				if (strncmp(galaxytype, "disk", 4) == 0)
				{
					...
					I_r_new[ifr] = (I_disk_r0_i * exp(-radius_store[ifr])
							/ r0disk_kpc_i);
...
double cum_integral(double radius[], double rstart, double rmax, double func[],
		int npoints)
{
