...
#define s_factor 1.0
#define arcsecstokpc 1  // at z=0.5 (arbisrary)
...
#define ntypes 9 // No. of morphology types of models
   ...
   for (pix=0; pix<npixels; pix++)
    ...
    /* Scale, incline and rotate galaxy with these parameters */
	 for (ifz=0; ifz<npoints_disk;ifz++){
	   ...
	   */
	   x_scaled=  x_disk[ifz] * s_factor * scaling_disk;  //kpc
	   y_scaled = y_disk[ifz] * s_factor * scaling_disk ;  //kpc
	   z_scaled = z_disk[ifz] * s_factor * scaling_disk;  //kpc
...
/*re-initialize galaxy_image_filter1 and 2 */
    ...
    for (ifz=0; ifz<npoints_bulge;ifz++){
	   ...
	   x_scaled = x_bulge[ifz] * scaling_bulge * s_factor ;  //from kpc 
	   y_scaled = y_bulge[ifz] * scaling_bulge * s_factor;  //from kpc	  
   ...
   for (pix=0; pix<npixels; pix++)
    ...
    /* Scale, incline and rotate galaxy with these parameters */
	 for (ifz=0; ifz<npoints_disk;ifz++){
	   ...
	   /* Scale x, y, z by same factors */
	   /*
	   x_scaled= (c_lambda - (d_lambda*log10(cos(theta_rad)))) * x_disk[ifz] * s_factor * scaling_disk;  //kpc
	   y_scaled = (c_lambda - (d_lambda*log10(cos(theta_rad)))) *  y_disk[ifz] * s_factor * scaling_disk ;  //kpc
	   z_scaled = (c_lambda - (d_lambda*log10(cos(theta_rad)))) * z_disk[ifz] * s_factor * scaling_disk ;  //kpc
	   */
...
int main(int argc, char *argv[]) 
 { 
   ...
   int objid;
   double ra, dec, dummy2, dummy3,dummy4,dummy5,dummy6,dummy7,dummy8,dummy9,dummy10,dummy11,dummy12,dummy13,dummy14,dummy15, dummy16, dummy17, zphot,zspec, R50_data_filter1, R50_data_filter2, R50_data_filter3, R50_data_filter4, R50_bestmodel_filter1, R50_bestmodel_filter2,  R50_bestmodel_filter3,  R50_bestmodel_filter4, R50_catalog, age_estimated, age_grid, R50, reff_bulge_arcsec, reff_disk_arcsec;
   ...
   double chi2_best_type, scaling_bulge_best_global, scaling_disk_best_global, e1_best_global, e2_best_global, xcentre_best_global,ycentre_best_global,  theta_rad_best_global, posangle_rad_best_global;
   ...
   bestchi2file=fopen("bestchi2_photoz_bvizyjh_bulge_disk_8kpcarcsec_test_ALL","a");

   /*
   chi2file_type0 = fopen("chi2_photoz_bvizyjh_bulge_disk_n4.0","a");
   chi2file_type1 = fopen("chi2_photoz_bvizyjh_bulge_disk_n3.5","a");
   chi2file_type2 = fopen("chi2_photoz_bvizyjh_bulge_disk_n3.0","a");
   chi2file_type3 = fopen("chi2_photoz_bvizyjh_bulge_disk_n2.5","a");
   chi2file_type4 = fopen("chi2_photoz_bvizyjh_bulge_disk_n2.0","a");
   chi2file_type5 = fopen("chi2_photoz_bvizyjh_bulge_disk_n1.5","a");
   chi2file_type6 = fopen("chi2_photoz_bvizyjh_bulge_disk_n1.0","a");
   chi2file_type7 = fopen("chi2_photoz_bvizyjh_bulge_disk_n0.5","a");
   */
...
for (pix=0; pix<npixels_wfc3;pix++)
 ...
 for (pix=0; pix<npixels_wfc3;pix++)
   ...
   //count+1, ra(ips),dec(ips), zspec_3dhst(mid(ips)),zphot(ips), R50(ips),VAB(ips),IAB(ips),M_I(ips),
   while(fscanf(fpcat,"%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf",&objid,&ra,&dec,&zspec,&zphot,&R50,&VAB,&IAB,&M_I)==9)
 ...
 for (ipoints=0;ipoints<npoints_disk;ipoints++)
  ...
  for (itype=0;itype<ntypes;itype++)
    {
      ...
      if (itype==0) {beta0 =beta0_n4;}
      if (itype==1) {beta0 =beta0_n3half;}
      if (itype==2) {beta0 =beta0_n3;}
      if (itype==3) {beta0 =beta0_n2half;}
      if (itype==4) {beta0 =beta0_n2;}
      if (itype==5) {beta0 =beta0_n1half;}
      if (itype==6) {beta0 =beta0_n1;}
      if (itype==7) {beta0 =beta0_nhalf;}
      ...
      for (ipoints=0;ipoints<npoints_bulge;ipoints++)
	{
      ...
      scaling_bulge=0.5; //0.05
      scaling_disk=0.5; //0.05
      e1 = 0.5;  //-0.5
      e2= 0.5;   //0.5
      xcentre = 0.05; //0
      ycentre = 0.05;   //0
      ...
      /*
      scaling_bulge = rvals2[0];
      scaling_disk = rvals2[1];
      e1 = rvals2[2];
      e2 = rvals2[3];  
      xcentre =rvals2[4];
      ycentre =rvals2[5];
           
      scaling_step =  0.1 ;//0.005; 
      e1_step=0.05;
      e2_step=0.05;
      xcentre_step=0.05;
      ycentre_step=0.05;
                     
      chi2_best_type=fitjointmodel(scaling_bulge,scaling_disk,e1, e2, xcentre, ycentre, scaling_step,e1_step,e2_step,xcentre_step, ycentre_step, rvals3);
      */
      ...
      /* store chi2, bulge and disk amplitudes and their errors for each type */

        chi2_type[itype] = chi2_best_type ;

    
        amp_bulge[itype][0] = amp_bulge_type_filter1;
	amp_bulge[itype][1] = amp_bulge_type_filter2;
	amp_bulge[itype][2] = amp_bulge_type_filter3;
	amp_bulge[itype][3] = amp_bulge_type_filter4;
	amp_bulge[itype][4] = amp_bulge_type_filter5;
	amp_bulge[itype][5] = amp_bulge_type_filter6;
	amp_bulge[itype][6] = amp_bulge_type_filter7;
	
	amp_disk[itype][0] = amp_disk_type_filter1;
	amp_disk[itype][1] = amp_disk_type_filter2;
	amp_disk[itype][2] = amp_disk_type_filter3;
	amp_disk[itype][3] = amp_disk_type_filter4;
	amp_disk[itype][4] = amp_disk_type_filter5;
	amp_disk[itype][5] = amp_disk_type_filter6;
	amp_disk[itype][6] = amp_disk_type_filter7;

	/*
        if (itype==0) { fprintf(chi2file_type0,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",objid, scaling_bulge_best, scaling_disk_best,  e1_best, e1_best, xcentre_best, ycentre_best, amp_bulge[itype][0], amp_bulge[itype][1], amp_bulge[itype][2], amp_bulge[itype][3], amp_bulge[itype][4], amp_bulge[itype][5], amp_bulge[itype][6], amp_disk[itype][0], amp_disk[itype][1], amp_disk[itype][2], amp_disk[itype][3], amp_disk[itype][4], amp_disk[itype][5], amp_disk[itype][6] , chi2_best_type); } //22 args

	 if (itype==1) { fprintf(chi2file_type1,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",objid, scaling_bulge_best, scaling_disk_best,  e1_best, e1_best, xcentre_best, ycentre_best, amp_bulge[itype][0], amp_bulge[itype][1], amp_bulge[itype][2], amp_bulge[itype][3], amp_bulge[itype][4], amp_bulge[itype][5], amp_bulge[itype][6], amp_disk[itype][0], amp_disk[itype][1], amp_disk[itype][2], amp_disk[itype][3], amp_disk[itype][4], amp_disk[itype][5], amp_disk[itype][6] , chi2_best_type); } //22 args

	 if (itype==2) { fprintf(chi2file_type2,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",objid, scaling_bulge_best, scaling_disk_best,  e1_best, e1_best, xcentre_best, ycentre_best, amp_bulge[itype][0], amp_bulge[itype][1], amp_bulge[itype][2], amp_bulge[itype][3], amp_bulge[itype][4], amp_bulge[itype][5], amp_bulge[itype][6], amp_disk[itype][0], amp_disk[itype][1], amp_disk[itype][2], amp_disk[itype][3], amp_disk[itype][4], amp_disk[itype][5], amp_disk[itype][6] , chi2_best_type); } //22 args
	 
	  if (itype==3) { fprintf(chi2file_type3,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",objid, scaling_bulge_best, scaling_disk_best,  e1_best, e1_best, xcentre_best, ycentre_best, amp_bulge[itype][0], amp_bulge[itype][1], amp_bulge[itype][2], amp_bulge[itype][3], amp_bulge[itype][4], amp_bulge[itype][5], amp_bulge[itype][6], amp_disk[itype][0], amp_disk[itype][1], amp_disk[itype][2], amp_disk[itype][3], amp_disk[itype][4], amp_disk[itype][5], amp_disk[itype][6] , chi2_best_type); } //22 args

	   if (itype==4) { fprintf(chi2file_type4,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",objid, scaling_bulge_best, scaling_disk_best,  e1_best, e1_best, xcentre_best, ycentre_best, amp_bulge[itype][0], amp_bulge[itype][1], amp_bulge[itype][2], amp_bulge[itype][3], amp_bulge[itype][4], amp_bulge[itype][5], amp_bulge[itype][6], amp_disk[itype][0], amp_disk[itype][1], amp_disk[itype][2], amp_disk[itype][3], amp_disk[itype][4], amp_disk[itype][5], amp_disk[itype][6] , chi2_best_type); } //22 args

	   if (itype==5) { fprintf(chi2file_type5,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",objid, scaling_bulge_best, scaling_disk_best,  e1_best, e1_best, xcentre_best, ycentre_best, amp_bulge[itype][0], amp_bulge[itype][1], amp_bulge[itype][2], amp_bulge[itype][3], amp_bulge[itype][4], amp_bulge[itype][5], amp_bulge[itype][6], amp_disk[itype][0], amp_disk[itype][1], amp_disk[itype][2], amp_disk[itype][3], amp_disk[itype][4], amp_disk[itype][5], amp_disk[itype][6] , chi2_best_type); } //22 args

	     if (itype==6) { fprintf(chi2file_type6,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",objid, scaling_bulge_best, scaling_disk_best,  e1_best, e1_best, xcentre_best, ycentre_best, amp_bulge[itype][0], amp_bulge[itype][1], amp_bulge[itype][2], amp_bulge[itype][3], amp_bulge[itype][4], amp_bulge[itype][5], amp_bulge[itype][6], amp_disk[itype][0], amp_disk[itype][1], amp_disk[itype][2], amp_disk[itype][3], amp_disk[itype][4], amp_disk[itype][5], amp_disk[itype][6] , chi2_best_type); } //22 args

	      if (itype==7) { fprintf(chi2file_type7,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",objid, scaling_bulge_best, scaling_disk_best,  e1_best, e1_best, xcentre_best, ycentre_best, amp_bulge[itype][0], amp_bulge[itype][1], amp_bulge[itype][2], amp_bulge[itype][3], amp_bulge[itype][4], amp_bulge[itype][5], amp_bulge[itype][6], amp_disk[itype][0], amp_disk[itype][1], amp_disk[itype][2], amp_disk[itype][3], amp_disk[itype][4], amp_disk[itype][5], amp_disk[itype][6] , chi2_best_type); } //22 args
	*/
  ...
  reff_bulge_arcsec = s_factor * scaling_bulge_best_global;
  reff_disk_arcsec = s_factor * scaling_disk_best_global;
	...
	model_conv_disk_filter7[pix] = model_conv_wfc3[pix];
	  ...
	  for (pix=0;pix<npixels_acs;pix++)
	    {
	       ...
	       sumflux_bulge_filter1 += (amp_bulge_global_filter1*model_conv_bulge_filter1[pix]) ;
	       sumflux_bulge_filter2 += (amp_bulge_global_filter2* model_conv_bulge_filter2[pix]) ;
	       sumflux_bulge_filter3 += (amp_bulge_global_filter3*model_conv_bulge_filter3[pix]) ;
	       sumflux_bulge_filter4 += (amp_bulge_global_filter4*model_conv_bulge_filter4[pix]) ;
		  
	       sumflux_disk_filter1 += (amp_disk_global_filter1*model_conv_disk_filter1[pix]) ;
	       sumflux_disk_filter2 += (amp_disk_global_filter2*model_conv_disk_filter2[pix]) ;
	       sumflux_disk_filter3 += (amp_disk_global_filter3*model_conv_disk_filter3[pix]) ;
	       sumflux_disk_filter4 += (amp_disk_global_filter4*model_conv_disk_filter4[pix]) ;
	     
	       sumerr_bulge_filter1 += pow(amp_err_bulge_global_filter1*model_conv_bulge_filter1[pix],2);
	       sumerr_bulge_filter2 += pow(amp_err_bulge_global_filter2*model_conv_bulge_filter2[pix],2);
	       sumerr_bulge_filter3 += pow (amp_err_bulge_global_filter3*model_conv_bulge_filter3[pix],2);
	       sumerr_bulge_filter4 += pow(amp_err_bulge_global_filter4*model_conv_bulge_filter4[pix],2);

	       sumerr_disk_filter1 += pow(amp_err_disk_global_filter1*model_conv_disk_filter1[pix],2);
	       sumerr_disk_filter2 += pow(amp_err_disk_global_filter2*model_conv_disk_filter2[pix],2);
	       sumerr_disk_filter3 += pow(amp_err_disk_global_filter3*model_conv_disk_filter3[pix],2);
	       sumerr_disk_filter4 += pow(amp_err_disk_global_filter4*model_conv_disk_filter4[pix],2);
	   ...
	   for (pix=0;pix<npixels_wfc3;pix++)
	    {
	       ...
	       sumflux_bulge_filter5 += (amp_bulge_global_filter5*model_conv_bulge_filter5[pix]) ;
	       sumflux_bulge_filter6 += (amp_bulge_global_filter6* model_conv_bulge_filter6[pix]) ;
	       sumflux_bulge_filter7 += (amp_bulge_global_filter7*model_conv_bulge_filter7[pix]) ;
		  
	       sumflux_disk_filter5 += (amp_disk_global_filter5*model_conv_disk_filter5[pix]) ;
	       sumflux_disk_filter6 += (amp_disk_global_filter6*model_conv_disk_filter6[pix]) ;
	       sumflux_disk_filter7 += (amp_disk_global_filter7*model_conv_disk_filter7[pix]) ;
	     
	       sumerr_bulge_filter5 += pow(amp_err_bulge_global_filter5*model_conv_bulge_filter5[pix],2);
	       sumerr_bulge_filter6 += pow(amp_err_bulge_global_filter6*model_conv_bulge_filter6[pix],2);
	       sumerr_bulge_filter7 += pow(amp_err_bulge_global_filter7*model_conv_bulge_filter7[pix],2);

	       sumerr_disk_filter5 += pow(amp_err_disk_global_filter5*model_conv_disk_filter5[pix],2);
	       sumerr_disk_filter6 += pow(amp_err_disk_global_filter6*model_conv_disk_filter6[pix],2);
	       sumerr_disk_filter7 += pow(amp_err_disk_global_filter7*model_conv_disk_filter7[pix],2);
	  ...
	  for (tt=0;tt<ncols_tot_acs;tt++) {
	      ...
	      weight_filter1[pix] = ( (1/(reff_disk_arcsec * sqrt(2 * M_PI))) * exp(- (  ((tt-x0_bestmodel_acs)*(tt-x0_bestmodel_acs) * pixelsize_arcsec_acs * pixelsize_arcsec_acs) + ((ss-y0_bestmodel_acs)*(ss-y0_bestmodel_acs) * pixelsize_arcsec_acs * pixelsize_arcsec_acs) ) /(2*reff_disk_arcsec*reff_disk_arcsec) )   )/pow(simrms_filter1[pix],2) ;

	      weight_filter2[pix] = ( (1/(reff_disk_arcsec * sqrt(2 * M_PI))) * exp(- (  ((tt-x0_bestmodel_acs)*(tt-x0_bestmodel_acs) * pixelsize_arcsec_acs * pixelsize_arcsec_acs) + ((ss-y0_bestmodel_acs)*(ss-y0_bestmodel_acs) * pixelsize_arcsec_acs * pixelsize_arcsec_acs) ) /(2*reff_disk_arcsec*reff_disk_arcsec) )   )/pow(simrms_filter2[pix],2) ;

	      weight_filter3[pix] = ( (1/(reff_disk_arcsec * sqrt(2 * M_PI))) * exp(- (  ((tt-x0_bestmodel_acs)*(tt-x0_bestmodel_acs) * pixelsize_arcsec_acs * pixelsize_arcsec_acs) + ((ss-y0_bestmodel_acs)*(ss-y0_bestmodel_acs) * pixelsize_arcsec_acs * pixelsize_arcsec_acs) ) /(2*reff_disk_arcsec*reff_disk_arcsec) )   )/pow(simrms_filter3[pix],2) ;

	      weight_filter4[pix] = ( (1/(reff_disk_arcsec * sqrt(2 * M_PI))) * exp(- (  ((tt-x0_bestmodel_acs)*(tt-x0_bestmodel_acs) * pixelsize_arcsec_acs * pixelsize_arcsec_acs) + ((ss-y0_bestmodel_acs)*(ss-y0_bestmodel_acs) * pixelsize_arcsec_acs * pixelsize_arcsec_acs) ) /(2*reff_disk_arcsec*reff_disk_arcsec) )   )/pow(simrms_filter4[pix],2) ;
	     
	      

	      if (radius_pix <= (reff_bulge_arcsec))
                {
		  ...
		  npix_inside_aperture_acs++;
              ...
              if ( (radius_pix > (reff_bulge_arcsec)) && (radius_pix <= (reff_disk_arcsec)))
		{
		  ...
		  npix_outside_aperture_acs++;
	       ...
	       if (radius_pix <= (reff_disk_arcsec))
		{
	  ...
	  for (tt=0;tt<ncols_tot_wfc3;tt++) {
	      ...
	      weight_filter5[pix] = ( (1/(reff_disk_arcsec * sqrt(2 * M_PI))) * exp(- (  ((tt-x0_bestmodel_wfc3)*(tt-x0_bestmodel_wfc3) * pixelsize_arcsec_wfc3 * pixelsize_arcsec_wfc3) + ((ss-y0_bestmodel_wfc3)*(ss-y0_bestmodel_wfc3) * pixelsize_arcsec_wfc3 * pixelsize_arcsec_wfc3) ) /(2*reff_disk_arcsec*reff_disk_arcsec) )   )/pow(simrms_filter5[pix],2) ;
	      weight_filter6[pix] = ( (1/(reff_disk_arcsec * sqrt(2 * M_PI))) * exp(- (  ((tt-x0_bestmodel_wfc3)*(tt-x0_bestmodel_wfc3) * pixelsize_arcsec_wfc3 * pixelsize_arcsec_wfc3) + ((ss-y0_bestmodel_wfc3)*(ss-y0_bestmodel_wfc3) * pixelsize_arcsec_wfc3 * pixelsize_arcsec_wfc3) ) /(2*reff_disk_arcsec*reff_disk_arcsec) )   )/pow(simrms_filter6[pix],2) ;
	      weight_filter7[pix] = ( (1/(reff_disk_arcsec * sqrt(2 * M_PI))) * exp(- (  ((tt-x0_bestmodel_wfc3)*(tt-x0_bestmodel_wfc3) * pixelsize_arcsec_wfc3 * pixelsize_arcsec_wfc3) + ((ss-y0_bestmodel_wfc3)*(ss-y0_bestmodel_wfc3) * pixelsize_arcsec_wfc3 * pixelsize_arcsec_wfc3) ) /(2*reff_disk_arcsec*reff_disk_arcsec) )   )/pow(simrms_filter7[pix],2) ;
	  ...
	  /* ACS */
	   for (tt=0;tt<ncols_tot_acs;tt++) {
		  ...
		  sumflux_model_inside_filter3  += (model_conv_summed_filter3[pix]);
                  ...
                  sumrms_data_inside_filter1 += pow(weight_filter1[pix]*simrms_filter1[pix],2)  ;
                  sumrms_data_inside_filter2 += pow(weight_filter2[pix]*simrms_filter2[pix],2)  ;
		  sumrms_data_inside_filter3 += pow(weight_filter3[pix]*simrms_filter3[pix],2)  ;
                  sumrms_data_inside_filter4 += pow(weight_filter4[pix]*simrms_filter4[pix],2)  ;
                }
		  ...
		  sumflux_model_outside_filter3  += (model_conv_summed_filter3[pix]);
                  ...
                  sumrms_data_outside_filter1 += pow(weight_filter1[pix]*simrms_filter1[pix],2)  ;
                  sumrms_data_outside_filter2 += pow(weight_filter2[pix]*simrms_filter2[pix],2)  ;
		  sumrms_data_outside_filter3 += pow(weight_filter3[pix]*simrms_filter3[pix],2)  ;
                  sumrms_data_outside_filter4 += pow(weight_filter4[pix]*simrms_filter4[pix],2)  ;
		  ...
		  sumrms_data_galaxy_filter1 += pow(weight_filter1[pix]*simrms_filter1[pix],2)  ;
                  sumrms_data_galaxy_filter2 += pow(weight_filter2[pix]*simrms_filter2[pix],2)  ;
		  sumrms_data_galaxy_filter3 += pow(weight_filter3[pix]*simrms_filter3[pix],2)  ;
                  sumrms_data_galaxy_filter4 += pow(weight_filter4[pix]*simrms_filter4[pix],2)  ;
		}
	    ...
	    for (tt=0;tt<ncols_tot_wfc3;tt++) {
		  ...
		  sumflux_model_inside_filter7  += (model_conv_summed_filter7[pix]);
                  ...
                  sumrms_data_inside_filter5 += pow(weight_filter5[pix]*simrms_filter5[pix],2)  ;
                  sumrms_data_inside_filter6 += pow(weight_filter6[pix]*simrms_filter6[pix],2)  ;
		  sumrms_data_inside_filter7 += pow(weight_filter7[pix]*simrms_filter7[pix],2)  ;
                }
		  ...
		  sumflux_model_outside_filter7  += (model_conv_summed_filter7[pix]);
                  ...
                  sumrms_data_outside_filter5 += pow(weight_filter5[pix]*simrms_filter5[pix],2)  ;
                  sumrms_data_outside_filter6 += pow(weight_filter6[pix]*simrms_filter6[pix],2)  ;
		  sumrms_data_outside_filter7 += pow(weight_filter7[pix]*simrms_filter7[pix],2)  ;
		  ...
		  sumrms_data_galaxy_filter5 += pow(weight_filter5[pix]*simrms_filter5[pix],2)  ;
                  sumrms_data_galaxy_filter6 += pow(weight_filter6[pix]*simrms_filter6[pix],2)  ;
		  sumrms_data_galaxy_filter7 += pow(weight_filter7[pix]*simrms_filter7[pix],2)  ;
		}
	...
	sumflux_data_inside_filter1 = npix_inside_aperture_acs * sqrt(sumflux_data_inside_filter1)/sum_weights_inside_filter1 ;
        sumflux_data_inside_filter2 = npix_inside_aperture_acs * sqrt(sumflux_data_inside_filter2)/sum_weights_inside_filter2 ;
	sumflux_data_inside_filter3 = npix_inside_aperture_acs * sqrt(sumflux_data_inside_filter3)/sum_weights_inside_filter3 ;
        sumflux_data_inside_filter4 = npix_inside_aperture_acs * sqrt(sumflux_data_inside_filter4)/sum_weights_inside_filter4 ;
	sumflux_data_inside_filter5 = npix_inside_aperture_wfc3 * sqrt(sumflux_data_inside_filter5)/sum_weights_inside_filter5 ;
	sumflux_data_inside_filter6 = npix_inside_aperture_wfc3 * sqrt(sumflux_data_inside_filter6)/sum_weights_inside_filter6 ;
        sumflux_data_inside_filter7 = npix_inside_aperture_wfc3 * sqrt(sumflux_data_inside_filter7)/sum_weights_inside_filter7 ;
	
        sumflux_data_outside_filter1 = npix_outside_aperture_acs * sqrt(sumflux_data_outside_filter1)/sum_weights_outside_filter1 ;
        sumflux_data_outside_filter2 = npix_outside_aperture_acs * sqrt(sumflux_data_outside_filter2)/sum_weights_outside_filter2;
	sumflux_data_outside_filter3 = npix_outside_aperture_acs * sqrt(sumflux_data_outside_filter3)/sum_weights_outside_filter3;
        sumflux_data_outside_filter4 = npix_outside_aperture_acs * sqrt(sumflux_data_outside_filter4)/sum_weights_outside_filter4;
	sumflux_data_outside_filter5 = npix_outside_aperture_wfc3 * sqrt(sumflux_data_outside_filter5)/sum_weights_outside_filter5;
	sumflux_data_outside_filter6 = npix_outside_aperture_wfc3 * sqrt(sumflux_data_outside_filter6)/sum_weights_outside_filter6;
	sumflux_data_outside_filter7 = npix_outside_aperture_wfc3 * sqrt(sumflux_data_outside_filter7)/sum_weights_outside_filter7;
	
	sumflux_data_galaxy_filter1 = npix_total_aperture_acs * sqrt(sumflux_data_galaxy_filter1)/sum_weights_galaxy_filter1;
        sumflux_data_galaxy_filter2 =  npix_total_aperture_acs *sqrt(sumflux_data_galaxy_filter2)/sum_weights_galaxy_filter2;
	sumflux_data_galaxy_filter3 =  npix_total_aperture_acs *sqrt(sumflux_data_galaxy_filter3)/sum_weights_galaxy_filter3;
        sumflux_data_galaxy_filter4 =  npix_total_aperture_acs *sqrt(sumflux_data_galaxy_filter4)/sum_weights_galaxy_filter4;
	sumflux_data_galaxy_filter5 =  npix_total_aperture_wfc3 *sqrt(sumflux_data_galaxy_filter5)/sum_weights_galaxy_filter5;
	sumflux_data_galaxy_filter6 =  npix_total_aperture_wfc3 *sqrt(sumflux_data_galaxy_filter6)/sum_weights_galaxy_filter6;
	sumflux_data_galaxy_filter7 =  npix_total_aperture_wfc3 *sqrt(sumflux_data_galaxy_filter7)/sum_weights_galaxy_filter7;
	    
	    
	sumrms_data_inside_filter1 = npix_inside_aperture_acs * sqrt(sumrms_data_inside_filter1)/sum_weights_inside_filter1 ;
        sumrms_data_inside_filter2 = npix_inside_aperture_acs * sqrt(sumrms_data_inside_filter2)/sum_weights_inside_filter2 ;
	sumrms_data_inside_filter3 = npix_inside_aperture_acs * sqrt(sumrms_data_inside_filter3)/sum_weights_inside_filter3 ;
        sumrms_data_inside_filter4 = npix_inside_aperture_acs * sqrt(sumrms_data_inside_filter4)/sum_weights_inside_filter4 ;
	sumrms_data_inside_filter5 = npix_inside_aperture_wfc3 * sqrt(sumrms_data_inside_filter5)/sum_weights_inside_filter5 ;
	sumrms_data_inside_filter6 = npix_inside_aperture_wfc3 * sqrt(sumrms_data_inside_filter6)/sum_weights_inside_filter6 ;
        sumrms_data_inside_filter7 = npix_inside_aperture_wfc3 * sqrt(sumrms_data_inside_filter7)/sum_weights_inside_filter7 ;
	
        sumrms_data_outside_filter1 = npix_outside_aperture_acs * sqrt(sumrms_data_outside_filter1)/sum_weights_outside_filter1 ;
        sumrms_data_outside_filter2 = npix_outside_aperture_acs *  sqrt(sumrms_data_outside_filter2)/sum_weights_outside_filter2;
	sumrms_data_outside_filter3 = npix_outside_aperture_acs * sqrt(sumrms_data_outside_filter3)/sum_weights_outside_filter3;
        sumrms_data_outside_filter4 = npix_outside_aperture_acs * sqrt(sumrms_data_outside_filter4)/sum_weights_outside_filter4;
	sumrms_data_outside_filter5 = npix_outside_aperture_wfc3 * sqrt(sumrms_data_outside_filter5)/sum_weights_outside_filter5;
	sumrms_data_outside_filter6 = npix_outside_aperture_wfc3 * sqrt(sumrms_data_outside_filter6)/sum_weights_outside_filter6;
	sumrms_data_outside_filter7 = npix_outside_aperture_wfc3 * sqrt(sumrms_data_outside_filter7)/sum_weights_outside_filter7;
	
	sumrms_data_galaxy_filter1 = npix_total_aperture_acs * sqrt(sumrms_data_galaxy_filter1)/sum_weights_galaxy_filter1;
        sumrms_data_galaxy_filter2 = npix_total_aperture_acs * sqrt(sumrms_data_galaxy_filter2)/sum_weights_galaxy_filter2;
	sumrms_data_galaxy_filter3 = npix_total_aperture_acs * sqrt(sumrms_data_galaxy_filter3)/sum_weights_galaxy_filter3;
        sumrms_data_galaxy_filter4 = npix_total_aperture_acs * sqrt(sumrms_data_galaxy_filter4)/sum_weights_galaxy_filter4;
	sumrms_data_galaxy_filter5 = npix_total_aperture_wfc3 * sqrt(sumrms_data_galaxy_filter5)/sum_weights_galaxy_filter5;
	sumrms_data_galaxy_filter6 = npix_total_aperture_wfc3 * sqrt(sumrms_data_galaxy_filter6)/sum_weights_galaxy_filter6;
	sumrms_data_galaxy_filter7 = npix_total_aperture_wfc3 * sqrt(sumrms_data_galaxy_filter7)/sum_weights_galaxy_filter7;
	...
	*/		   
	...
	fprintf(bestchi2file,"%d %10.7lf %10.7lf %lf %lf %lf %lf %lf %d %lf %lf %lf %lf %lf %lf %lf %lf\n",objid,ra,dec,zspec,zphot,VAB,IAB,M_I,itype_best_global,reff_bulge_arcsec, reff_disk_arcsec,e1_best_global, e2_best_global, xcentre_best_global, ycentre_best_global, amp_bulge_global_filter3/(amp_bulge_global_filter3+amp_disk_global_filter3),chi2_best_global/Nfreedom); // 17 arguments
	...
	strcpy(outputmapspath,"../results/maps_bvizyjh_zphot/");
