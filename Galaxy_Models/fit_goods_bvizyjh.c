...
double *apix, *simdata_filter1, *simdata_filter2, *simdata_filter3,
		*simdata_filter4, *simdata_filter5, *simdata_filter6, *simdata_filter7,
		*psf_image_filter1, *psf_image_filter2, *psf_image_filter3,
		*psf_image_filter4, *psf_image_filter5, *psf_image_filter6,
		*psf_image_filter7, *simrms_filter1, *simrms_filter2, *simrms_filter3,
		*simrms_filter4, *simrms_filter5, *simrms_filter6, *simrms_filter7,
		*model_conv_acs, *model_conv_wfc3, *model_unconv_acs,
		*model_unconv_wfc3, *psf_image_acs, *psf_image_wfc3;
double *in_acs, *in_wfc3, *identity_acs, *identity_wfc3, *final_acs,
		*final_wfc3;
fftw_complex *inTrans_acs, *inTrans_wfc3, *identityTrans_acs,
		*identityTrans_wfc3, *FinalFFT_acs, *FinalFFT_wfc3;
fftw_plan plan1_acs, plan1_wfc3, plan2_acs, plan2_wfc3, plan3_acs, plan3_wfc3;

double *x_bulge, *y_bulge, *z_bulge, *I_xyz_bulge_filter1, *I_xyz_disk_filter1,
		*x_disk, *y_disk, *z_disk, f2f1_ratio_disk_type, f2f1_ratio_bulge_type;
...
int w, h;
double Nfreedom;
double amp_etype_type_filter1, amp_etype_type_filter2, amp_disk_type_filter1,
		amp_disk_type_filter2, amp_bulge_type_filter1, amp_bulge_type_filter2;
double amp_etype_type_filter3, amp_etype_type_filter4, amp_disk_type_filter3,
		amp_disk_type_filter4, amp_bulge_type_filter3, amp_bulge_type_filter4;
double amp_etype_type_filter5, amp_etype_type_filter6, amp_disk_type_filter5,
		amp_disk_type_filter6, amp_bulge_type_filter5, amp_bulge_type_filter6,
		amp_bulge_type_filter7, amp_disk_type_filter7;

/* Make unconvolved model on ACS pixel grid */
void make2Dgalaxymodelunconvolved_acs(int componenttype, int filtertype,
		double scaling, double e1, double e2, double xcentre, double ycentre,
		double arcsecstokpc, double weight_component)
{
	...
	{
		c_lambda =
				(l_lambda * 4350.0 * 4350.0) + (m_lambda * 4350.0) + n_lambda;
		d_lambda =
				(u_lambda * 4350.0 * 4350.0) + (v_lambda * 4350.0) + w_lambda;
	}
	if (filtertype == 1)
	{
		c_lambda =
				(l_lambda * 6060.0 * 6060.0) + (m_lambda * 6060.0) + n_lambda;
		d_lambda =
				(u_lambda * 6060.0 * 6060.0) + (v_lambda * 6060.0) + w_lambda;
	} //V
	if (filtertype == 2)
	{
		c_lambda =
				(l_lambda * 7750.0 * 7750.0) + (m_lambda * 7750.0) + n_lambda;
		d_lambda =
				(u_lambda * 7750.0 * 7750.0) + (v_lambda * 7750.0) + w_lambda;
	} //I
	if (filtertype == 3)
	{
		c_lambda =
				(l_lambda * 8500.0 * 8500.0) + (m_lambda * 8500.0) + n_lambda;
		d_lambda =
	...
	nrows_tot = Npix_side_acs;
	ncols_tot = Npix_side_acs;
	...
	if (componenttype == 1)
	{
		...
		for (ifz = 0; ifz < npoints_disk; ifz++)
		{
			...
			x_scaled = (c_lambda - (d_lambda * (2.5 * log10(cos(theta_rad)))))
					* x_disk[ifz] * 5 * scaling;  //kpc
			y_scaled = (c_lambda - (d_lambda * (2.5 * log10(cos(theta_rad)))))
					* y_disk[ifz] * 5 * scaling;  //kpc
			z_scaled = (c_lambda - (d_lambda * (2.5 * log10(cos(theta_rad)))))
					* z_disk[ifz] * 5 * scaling;  //kpc
			...
			x_inclined = x_scaled;
			y_inclined = (cos(theta_rad) * y_scaled)
					- (sin(theta_rad) * z_scaled);
			...
			y_final_disk = ((sin(posangle_rad) * x_inclined)
					+ (cos(posangle_rad) * y_inclined));

			/* find which pixel the particle falls into and do gridding of mass */

			i_index = ((int) (((x_final_disk + xcentre - xmin) / pixelsize_kpc)
					+ 1)) - 1; // pixel in x direction
			j_index = ((int) (((y_final_disk + ycentre - ymin) / pixelsize_kpc)
					+ 1)) - 1;  // pixel in y direction
			pix = (ncols_tot * j_index) + i_index;

			if (i_index >= 0 && i_index < ncols_tot && j_index >= 0
					&& j_index < nrows_tot)
			{
				model_unconv_acs[pix] = model_unconv_acs[pix]
						+ (I_xyz_disk_filter1[ifz]); // add the flux to the pixel
	...
	if (componenttype == 0)
	{
		...
		for (ifz = 0; ifz < npoints_bulge; ifz++)
		{
			...
			x_final_bulge = ((cos(posangle_rad) * x_inclined)
					- (sin(posangle_rad) * y_inclined));
			y_final_bulge = ((sin(posangle_rad) * x_inclined)
					+ (cos(posangle_rad) * y_inclined));

			i_index = ((int) (((x_final_bulge + xcentre - xmin) / pixelsize_kpc)
					+ 1)) - 1; // pixel in x direction
			j_index = ((int) (((y_final_bulge + ycentre - ymin) / pixelsize_kpc)
					+ 1)) - 1;  // pixel in y direction
			pix = (ncols_tot * j_index) + i_index;
			...
			{
				model_unconv_acs[pix] = model_unconv_acs[pix]
						+ (I_xyz_bulge_filter1[ifz]); // add the flux to the pixel
...
void make2Dgalaxymodelunconvolved_wfc3(int componenttype, int filtertype,
		double scaling, double e1, double e2, double xcentre, double ycentre,
		double arcsecstokpc, double weight_component)
{
	...
	if (filtertype == 4)
	{
		c_lambda = (l_lambda * 10500.0 * 10500.0) + (m_lambda * 10500.0)
				+ n_lambda;
		d_lambda = (u_lambda * 10500.0 * 10500.0) + (v_lambda * 10500.0)
				+ w_lambda;
	} //Y
	if (filtertype == 5)
	{
		c_lambda = (l_lambda * 12500.0 * 12500.0) + (m_lambda * 12500.0)
				+ n_lambda;
		d_lambda = (u_lambda * 12500.0 * 12500.0) + (v_lambda * 12500.0)
				+ w_lambda;
	} //J
	if (filtertype == 6)
	{
		c_lambda = (l_lambda * 16500.0 * 16500.0) + (m_lambda * 16500.0)
				+ n_lambda;
		d_lambda = (u_lambda * 16500.0 * 16500.0) + (v_lambda * 16500.0)
				+ w_lambda;
	} //H
	...
	nrows_tot = Npix_side_wfc3;
	ncols_tot = Npix_side_wfc3;
	...
	if (componenttype == 1)
	{
		...
		for (ifz = 0; ifz < npoints_disk; ifz++)
		{
			...
			if (i_index >= 0 && i_index < ncols_tot && j_index >= 0
					&& j_index < nrows_tot)
			{
				model_unconv_wfc3[pix] = model_unconv_wfc3[pix]
						+ (I_xyz_disk_filter1[ifz]); // add the flux to the pixel
				disk_flux_tot = disk_flux_tot + (I_xyz_disk_filter1[ifz]);
			}
...
void make2Dgalaxymodelconvolved_acs(int componenttype, int filtertype,
		double scaling, double e1, double e2, double xcentre, double ycentre,
		double arcsecstokpc, double weight_component)
{
	...
	int pheight, pwidth, imheight, imwidth, psfsize, imsize, padfactor,
			padwidth, padheight, padsize, complexwidth, complexheight,
			complexsize, padcomplexsize;
	double x_final_bulge, y_final_bulge, x_final_disk, y_final_disk, summodel;
	double disk_flux_tot = 0.0;
	double bulge_flux_tot = 0.0;
	double norm_disk;
	...
	for (i = 0; i < h * w; i++)
	{
		if (filtertype == 0)
		{
			psf_image_acs[i] = psf_image_filter1[i];
		}
		if (filtertype == 1)
		{
			psf_image_acs[i] = psf_image_filter2[i];
		}
		if (filtertype == 2)
		{
			psf_image_acs[i] = psf_image_filter3[i];
		}
		if (filtertype == 3)
		{
			psf_image_acs[i] = psf_image_filter4[i];
		}
	}
	...
	for (i = 0; i < h * w; i++)
		model_conv_acs[i] = model_conv_acs[i] * filterfactor * weight_component
				/ tot_flux;

}

// make PSF convolved galaxy model at WFC3 image
void make2Dgalaxymodelconvolved_wfc3(int componenttype, int filtertype,
		double scaling, double e1, double e2, double xcentre, double ycentre,
		double arcsecstokpc, double weight_component)
{
	double c_lambda, d_lambda;
	...
	for (i = 0; i < h * w; i++)
	{
		if (filtertype == 4)
		{
			psf_image_wfc3[i] = psf_image_filter5[i];
		}
		if (filtertype == 5)
		{
			psf_image_wfc3[i] = psf_image_filter6[i];
		}
		if (filtertype == 6)
		{
			psf_image_wfc3[i] = psf_image_filter7[i];
		}
	}
	...
	for (i = 0; i < h * w; i++)
	{
		model_conv_wfc3[i] = model_conv_wfc3[i] * filterfactor
				* weight_component / tot_flux;
	}
...
double jointlikel(gsl_vector *v, void *params)
{
	...
	double amp_bulge_filter1, amp_bulge_filter2, amp_bulge_filter3,
			amp_bulge_filter4, amp_bulge_filter5, amp_bulge_filter6,
			amp_bulge_filter7, amp_disk_filter1, amp_disk_filter2,
			amp_disk_filter3, amp_disk_filter4, amp_disk_filter5,
			amp_disk_filter6, amp_disk_filter7;
	double amp_etype_filter1, amp_etype_filter2, amp_etype_filter3,
			amp_etype_filter4, amp_etype_filter5, amp_etype_filter6,
			amp_etype_filter7, penalty_amp_bulge_filter1 = 0.0,
			penalty_amp_bulge_filter2 = 0.0, penalty_amp_bulge_filter3 = 0.0,
			penalty_amp_bulge_filter4 = 0.0, penalty_amp_bulge_filter5 = 0.0,
			penalty_amp_bulge_filter6 = 0.0, penalty_amp_bulge_filter7 = 0.0,
			penalty_amp_disk_filter1 = 0.0, penalty_amp_disk_filter2 = 0.0,
			penalty_amp_disk_filter3 = 0.0, penalty_amp_disk_filter4 = 0.0,
			penalty_amp_disk_filter5 = 0.0, penalty_amp_disk_filter6 = 0.0,
			penalty_amp_disk_filter7 = 0.0, penalty_amp_etype_filter1 = 0.0,
			penalty_amp_etype_filter2 = 0.0, penalty_amp_etype_filter3 = 0.0,
			penalty_amp_etype_filter4 = 0.0, penalty_amp_etype_filter5 = 0.0,
			penalty_amp_etype_filter6 = 0.0, penalty_amp_etype_filter7 = 0.0;
	...
	data_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_bulge_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_disk_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));

	data_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_bulge_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_disk_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));

	data_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_bulge_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_disk_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));

	data_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_bulge_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_disk_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));

	data_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_bulge_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_disk_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	data_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_bulge_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_disk_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	data_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_bulge_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_disk_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	...
	if (sqrt(e2 * e2 + e1 * e1) > 1.0)
	{
		penalty_e1e2quad = pow(npixels_acs - Nparams - 1, 4)
				* pow(sqrt(e2 * e2 + e1 * e1) - 1, 2);
		theta_e1e2 = atan2(e2, e1);
		e1 = 0.99 * cos(theta_e1e2);
		e2 = 0.99 * sin(theta_e1e2);
	}
	if (scaling <= 0.)
	{
		penalty_scaling = pow(npixels_acs - Nparams - 1, 4) * (scaling)
				* (scaling);
	}
	...
	make2Dgalaxymodelconvolved_acs(0, 0, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_acs); ll++)
		model_bulge_filter1[ll] = model_conv_acs[ll];

	make2Dgalaxymodelconvolved_acs(1, 0, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_acs); ll++)
		model_disk_filter1[ll] = model_conv_acs[ll];

	make2Dgalaxymodelconvolved_acs(0, 1, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_acs); ll++)
		model_bulge_filter2[ll] = model_conv_acs[ll];

	make2Dgalaxymodelconvolved_acs(1, 1, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_acs); ll++)
		model_disk_filter2[ll] = model_conv_acs[ll];

	make2Dgalaxymodelconvolved_acs(0, 2, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_acs); ll++)
		model_bulge_filter3[ll] = model_conv_acs[ll];

	make2Dgalaxymodelconvolved_acs(1, 2, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_acs); ll++)
		model_disk_filter3[ll] = model_conv_acs[ll];

	make2Dgalaxymodelconvolved_acs(0, 3, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_acs); ll++)
		model_bulge_filter4[ll] = model_conv_acs[ll];

	make2Dgalaxymodelconvolved_acs(1, 3, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_acs); ll++)
		model_disk_filter4[ll] = model_conv_acs[ll];

	make2Dgalaxymodelconvolved_wfc3(0, 4, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_wfc3); ll++)
		model_bulge_filter5[ll] = model_conv_wfc3[ll];

	make2Dgalaxymodelconvolved_wfc3(1, 4, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_wfc3); ll++)
		model_disk_filter5[ll] = model_conv_wfc3[ll];

	make2Dgalaxymodelconvolved_wfc3(0, 5, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_wfc3); ll++)
		model_bulge_filter6[ll] = model_conv_wfc3[ll];

	make2Dgalaxymodelconvolved_wfc3(1, 5, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_wfc3); ll++)
		model_disk_filter6[ll] = model_conv_wfc3[ll];

	make2Dgalaxymodelconvolved_wfc3(0, 6, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);

	for (ll = 0; ll < (npixels_wfc3); ll++)
		model_bulge_filter7[ll] = model_conv_wfc3[ll];

	make2Dgalaxymodelconvolved_wfc3(1, 6, scaling, e1, e2, xcentre, ycentre,
			arcsecstokpc, 1.0);
	...
	if (itype == 0)
	{
		...
		if (denom != 0.)
		{
			...
			for (j = 0; j < npixels_acs; j++)
			{
				lval += pow(
						(data_filter1[j]
								- (amp_etype_filter1 * model_bulge_filter1[j])),
						2);

			}

			if (amp_etype_filter1 < 0.0)
			{
				penalty_amp_etype_filter1 = 1e7;
			}
		...
		if (denom != 0.)
		{
			...
			for (j = 0; j < npixels_acs; j++)
			{
				lval += pow(
						(data_filter2[j]
								- (amp_etype_filter2 * model_bulge_filter2[j])),
						2);

			}

			if (amp_etype_filter2 < 0.0)
			{
				penalty_amp_etype_filter2 = 1e7;
			}
		...
		if (denom != 0.)
		{
			...
			for (j = 0; j < npixels_acs; j++)
			{
				lval += pow(
						(data_filter3[j]
								- (amp_etype_filter3 * model_bulge_filter3[j])),
						2);

			}
			if (amp_etype_filter3 < 0.0)
			{
				penalty_amp_etype_filter3 = 1e7;
			}
		...
		if (denom != 0.)
		{
			...
			for (j = 0; j < npixels_acs; j++)
			{
				lval += pow(
						(data_filter4[j]
								- (amp_etype_filter4 * model_bulge_filter4[j])),
						2);

			}

			if (amp_etype_filter4 < 0.0)
			{
				penalty_amp_etype_filter4 = 1e7;
			}
		...
		if (denom != 0.)
		{
			...
			for (j = 0; j < npixels_wfc3; j++)
			{
				lval += pow(
						(data_filter5[j]
								- (amp_etype_filter5 * model_bulge_filter5[j])),
						2);

			}

			if (amp_etype_filter5 < 0.0)
			{
				penalty_amp_etype_filter5 = 1e7;
			}
		...
		if (denom != 0.)
		{
			...
			for (j = 0; j < npixels_wfc3; j++)
			{
				lval += pow(
						(data_filter6[j]
								- (amp_etype_filter6 * model_bulge_filter6[j])),
						2);

			}

			if (amp_etype_filter6 < 0.0)
			{
				penalty_amp_etype_filter6 = 1e7;
			}
		...
		if (denom != 0.)
		{
			...
			for (j = 0; j < npixels_wfc3; j++)
			{
				lval += pow(
						(data_filter7[j]
								- (amp_etype_filter7 * model_bulge_filter7[j])),
						2);

			}

			if (amp_etype_filter7 < 0.0)
			{
				penalty_amp_etype_filter7 = 1e7;
			}
		...
		lval = lval + penalty_e1e2quad + penalty_scaling
				+ penalty_amp_etype_filter1 + penalty_amp_etype_filter2
				+ penalty_amp_etype_filter3 + penalty_amp_etype_filter4
				+ penalty_amp_etype_filter5 + penalty_amp_etype_filter6
				+ penalty_amp_etype_filter7;
	...
	if (itype > 0)
	{
				...
				lval =
						lval
								+ pow(
										data_filter1[j]
												- ((amp_disk_filter1
														* model_disk_filter1[j])
														+ (amp_bulge_filter1
																* model_bulge_filter1[j])),
										2);
				...
				lval =
						lval
								+ pow(
										data_filter2[j]
												- ((amp_disk_filter2
														* model_disk_filter2[j])
														+ (amp_bulge_filter2
																* model_bulge_filter2[j])),
										2);
				...
				lval =
						lval
								+ pow(
										data_filter3[j]
												- ((amp_disk_filter3
														* model_disk_filter3[j])
														+ (amp_bulge_filter3
																* model_bulge_filter3[j])),
										2);
				...
				lval =
						lval
								+ pow(
										data_filter4[j]
												- ((amp_disk_filter4
														* model_disk_filter4[j])
														+ (amp_bulge_filter4
																* model_bulge_filter4[j])),
										2);
				...
				lval =
						lval
								+ pow(
										data_filter5[j]
												- ((amp_disk_filter5
														* model_disk_filter5[j])
														+ (amp_bulge_filter5
																* model_bulge_filter5[j])),
										2);
				...
				lval =
						lval
								+ pow(
										data_filter6[j]
												- ((amp_disk_filter6
														* model_disk_filter6[j])
														+ (amp_bulge_filter6
																* model_bulge_filter6[j])),
										2);
				...
				lval =
						lval
								+ pow(
										data_filter7[j]
												- ((amp_disk_filter7
														* model_disk_filter7[j])
														+ (amp_bulge_filter7
																* model_bulge_filter7[j])),
										2);
		...
		if (amp_bulge_filter1 < 0.0)
		{
			penalty_amp_bulge_filter1 = 1e7;
		}
		if (amp_disk_filter1 < 0.0)
		{
			penalty_amp_disk_filter1 = 1e7;
		}
		if (amp_bulge_filter2 < 0.0)
		{
			penalty_amp_bulge_filter2 = 1e7;
		}
		if (amp_disk_filter2 < 0.0)
		{
			penalty_amp_disk_filter2 = 1e7;
		}
		if (amp_bulge_filter3 < 0.0)
		{
			penalty_amp_bulge_filter3 = 1e7;
		}
		if (amp_disk_filter3 < 0.0)
		{
			penalty_amp_disk_filter3 = 1e7;
		}
		if (amp_bulge_filter4 < 0.0)
		{
			penalty_amp_bulge_filter4 = 1e7;
		}
		if (amp_disk_filter4 < 0.0)
		{
			penalty_amp_disk_filter4 = 1e7;
		}
		if (amp_bulge_filter5 < 0.0)
		{
			penalty_amp_bulge_filter5 = 1e7;
		}
		if (amp_disk_filter5 < 0.0)
		{
			penalty_amp_disk_filter5 = 1e7;
		}
		if (amp_bulge_filter6 < 0.0)
		{
			penalty_amp_bulge_filter6 = 1e7;
		}
		if (amp_disk_filter6 < 0.0)
		{
			penalty_amp_disk_filter6 = 1e7;
		}
		if (amp_bulge_filter7 < 0.0)
		{
			penalty_amp_bulge_filter7 = 1e7;
		}
		if (amp_disk_filter7 < 0.0)
		{
			penalty_amp_disk_filter7 = 1e7;
		}
		...
		lval = lval + penalty_e1e2quad + penalty_scaling
				+ penalty_amp_bulge_filter1 + penalty_amp_disk_filter1
				+ penalty_amp_bulge_filter2 + penalty_amp_disk_filter2
				+ penalty_amp_bulge_filter3 + penalty_amp_disk_filter3
				+ penalty_amp_bulge_filter4 + penalty_amp_disk_filter4
				+ penalty_amp_bulge_filter5 + penalty_amp_disk_filter5
				+ penalty_amp_bulge_filter6 + penalty_amp_disk_filter6
				+ penalty_amp_bulge_filter7 + penalty_amp_disk_filter7; // + penalty_xcentre + penalty_ycentre ;
...
double fitjointmodel(double scaling, double e1, double e2, double xcentre,
		double ycentre, double scaling_step, double e1_step, double e2_step,
		double xcentre_step, double ycentre_step, double *rvals)
{
...
int main(int argc, char *argv[])
{
	...
	double sum_weights_inside_filter1, sum_weights_inside_filter2,
			sum_weights_outside_filter1, sum_weights_outside_filter2;
	double sum_weights_inside_filter3, sum_weights_inside_filter4,
			sum_weights_outside_filter3, sum_weights_outside_filter4;
	double sum_weights_inside_filter5, sum_weights_inside_filter6,
			sum_weights_outside_filter5, sum_weights_outside_filter6;
	double sum_weights_inside_filter7, sum_weights_outside_filter7;
	double *mask_filter1, *mask_filter2, *mask_filter3, *mask_filter4,
			*mask_filter5, *mask_filter6, *mask_filter7;

	int objid;
	double ra, dec, dummy2, dummy3, dummy4, dummy5, dummy6, dummy7, dummy8,
			dummy9, dummy10, dummy11, dummy12, dummy13, dummy14, dummy15,
			dummy16, dummy17, zphot, R50_data_filter1, R50_data_filter2,
			R50_data_filter3, R50_data_filter4, R50_bestmodel_filter1,
			R50_bestmodel_filter2, R50_bestmodel_filter3, R50_bestmodel_filter4,
			R50_catalog, age_estimated, age_grid, R50;
	double BAB, VAB, RAB, IAB, ZAB, M_B, M_V, M_R, M_I, M_Z;
	double *simdata_orig_filter1, *simdata_orig_filter2, *simdata_orig_filter3,
			*simdata_orig_filter4, *simdata_orig_filter5, *simdata_orig_filter6,
			*simdata_orig_filter7, *simnoise_filter1, *simnoise_filter2,
			*simnoise_filter3, *simnoise_filter4, *simnoise_filter5,
			*simnoise_filter6, *simnoise_filter7, *weight_filter1,
			*weight_filter2, *weight_filter3, *weight_filter4, *weight_filter5,
			*weight_filter6, *weight_filter7;
	int iter = 0;
	...
	double sum_weights_galaxy_filter4 = 0.0, sum_weights_galaxy_filter5 = 0.0,
			sum_weights_galaxy_filter6 = 0.0, sum_weights_galaxy_filter7 = 0.0;
	...
	double colour_data_inside, colour_data_outside, colour_model_inside,
			colour_model_outside;
	int tt = 0;
	int ss = 0;
	...
	double penalty_amp_etype_filter1 = 0.0, penalty_amp_etype_filter2 = 0.0,
			penalty_amp_bulge_filter1 = 0.0, penalty_amp_bulge_filter2 = 0.0,
			penalty_amp_disk_filter1 = 0.0, penalty_amp_disk_filter2 = 0.0;
	double penalty_amp_etype_filter3 = 0.0, penalty_amp_etype_filter4 = 0.0,
			penalty_amp_bulge_filter3 = 0.0, penalty_amp_bulge_filter4 = 0.0,
			penalty_amp_disk_filter3 = 0.0, penalty_amp_disk_filter4 = 0.0;
	double penalty_amp_etype_filter5 = 0.0, penalty_amp_etype_filter6 = 0.0,
			penalty_amp_bulge_filter5 = 0.0, penalty_amp_bulge_filter6 = 0.0,
			penalty_amp_disk_filter5 = 0.0, penalty_amp_disk_filter6 = 0.0;
	double penalty_amp_etype_filter7 = 0.0, penalty_amp_bulge_filter7 = 0.0,
			penalty_amp_disk_filter7 = 0.0;
	double btratio_filter1, btratio_filter2, btratio_filter3, btratio_filter4,
			btratio_filter5, btratio_filter6, btratio_filter7;

	double vi_central_pixel_data, vierr_central_pixel_data,
			vi_central_pixel_model_summed_unconvolved,
			vi_central_pixel_model_summed_convolved,
			vi_central_pixel_model_bulge_convolved,
			vi_central_pixel_model_bulge_unconvolved,
			vi_central_pixel_model_disk_convolved,
			vi_central_pixel_model_disk_unconvolved;
	int ipoints;
	double *data_filter1, *model_disk_filter1, *model_bulge_filter1,
			*data_filter2, *model_disk_filter2, *model_bulge_filter2;
	double *data_filter3, *model_disk_filter3, *model_bulge_filter3,
			*data_filter4, *model_disk_filter4, *model_bulge_filter4;
	double *data_filter5, *model_disk_filter5, *model_bulge_filter5,
			*data_filter6, *model_disk_filter6, *model_bulge_filter6;
	double *data_filter7, *model_disk_filter7, *model_bulge_filter7;

	double datamax_filter1, datamax_filter2, residual_filter1, residual_filter2,
			residualmax_filter1, residualmax_filter2;
	double datamax_filter3, datamax_filter4, residual_filter3, residual_filter4,
			residualmax_filter3, residualmax_filter4;
	double datamax_filter5, datamax_filter6, residual_filter5, residual_filter6,
			residualmax_filter5, residualmax_filter6;
	double datamax_filter7, residual_filter7, residualmax_filter7;

	double lval, sumx, sumy, sumxx, sumzz, sumyy, sumxy, sumzy, sumxz, amp_disk,
			amp_etype_filter1, amp_etype_filter2, denom, amp_bulge_filter1,
			amp_disk_filter1, amp_bulge_filter2, amp_disk_filter2,
			amp_bulge_global_filter1, amp_bulge_global_filter2,
			amp_disk_global_filter1, amp_disk_global_filter2;
	double amp_etype_filter3, amp_etype_filter4, amp_bulge_filter3,
			amp_disk_filter3, amp_bulge_filter4, amp_disk_filter4,
			amp_bulge_global_filter3, amp_bulge_global_filter4,
			amp_disk_global_filter3, amp_disk_global_filter4;
	double amp_etype_filter5, amp_etype_filter6, amp_bulge_filter5,
			amp_disk_filter5, amp_bulge_filter6, amp_disk_filter6,
			amp_bulge_global_filter5, amp_bulge_global_filter6,
			amp_disk_global_filter5, amp_disk_global_filter6;
	double amp_etype_filter7, amp_bulge_filter7, amp_disk_filter7,
			amp_bulge_global_filter7, amp_disk_global_filter7;
	double penalty_e1e2quad = 0.0, penalty_scaling = 0.0;
	double f2f1_bulge_e, f2f1_disk_e, f2f1_bulge_s0, f2f1_disk_s0,
			f2f1_bulge_sa, f2f1_disk_sa, f2f1_bulge_sb, f2f1_disk_sb,
			f2f1_bulge_sc, f2f1_disk_sc, f2f1_bulge_sd, f2f1_disk_sd;
	double dL, age_universe;
	double *cum_model_flux_radius_filter1, *cum_model_flux_radius_filter2,
			*cum_model_flux_radius_filter3, *cum_model_flux_radius_filter4,
			*cum_model_flux_radius_filter5, *cum_model_flux_radius_filter6,
			*cum_model_flux_radius_filter7;
	double *cum_data_flux_radius_filter1, *cum_data_flux_radius_filter2,
			*cum_data_flux_radius_filter3, *cum_data_flux_radius_filter4,
			*cum_data_flux_radius_filter5, *cum_data_flux_radius_filter6,
			*cum_data_flux_radius_filter7;
	double *cum_data_err_radius_filter1, *cum_data_err_radius_filter2,
			*cum_data_err_radius_filter3, *cum_data_err_radius_filter4,
			*cum_data_err_radius_filter5, *cum_data_err_radius_filter6,
			*cum_data_err_radius_filter7;
	double r0, radius_step, radius_max, fluxtot_data_filter1,
			fluxtot_data_filter2, fluxtot_model_filter1, fluxtot_model_filter2,
			radius_pix, cum_model_flux_istep_filter1,
			cum_model_flux_istep_filter2, cum_data_flux_istep_filter1,
			cum_data_flux_istep_filter2, r50_data_filter1, r50_data_filter2,
			r50_model_filter1, r50_model_filter2, maxvalue_filter1,
			maxvalue_filter2, cum_data_err_istep_filter1,
			cum_data_err_istep_filter2;
	double fluxtot_data_filter3, fluxtot_data_filter4, fluxtot_model_filter3,
			fluxtot_model_filter4, cum_model_flux_istep_filter3,
			cum_model_flux_istep_filter4, cum_data_flux_istep_filter3,
			cum_data_flux_istep_filter4, r50_data_filter3, r50_data_filter4,
			r50_model_filter3, r50_model_filter4, maxvalue_filter3,
			maxvalue_filter4, cum_data_err_istep_filter3,
			cum_data_err_istep_filter4;
	double fluxtot_data_filter5, fluxtot_data_filter6, fluxtot_model_filter5,
			fluxtot_model_filter6, cum_model_flux_istep_filter5,
			cum_model_flux_istep_filter6, cum_data_flux_istep_filter5,
			cum_data_flux_istep_filter6, r50_data_filter5, r50_data_filter6,
			r50_model_filter5, r50_model_filter6, maxvalue_filter5,
			maxvalue_filter6, cum_data_err_istep_filter5,
			cum_data_err_istep_filter6;
	double fluxtot_data_filter7, fluxtot_model_filter7,
			cum_model_flux_istep_filter7, cum_data_flux_istep_filter7,
			r50_data_filter7, r50_model_filter7, maxvalue_filter7,
			cum_data_err_istep_filter7;

	int x0_filter1, y0_filter1, x0_filter2, y0_filter2, x0_filter3, y0_filter3,
			x0_filter4, y0_filter4, x0_filter5, y0_filter5, x0_filter6,
			y0_filter6, x0_filter7, y0_filter7, nsteps, istep,
			istep_r50_data_filter1, istep_r50_data_filter2,
			istep_r50_model_filter1, istep_r50_model_filter2,
			istep_r50_data_filter3, istep_r50_data_filter4,
			istep_r50_model_filter3, istep_r50_model_filter4,
			istep_r50_data_filter5, istep_r50_data_filter6,
			istep_r50_model_filter5, istep_r50_model_filter6,
			istep_r50_data_filter7, istep_r50_model_filter7, x0_bestmodel_acs,
			y0_bestmodel_acs, x0_bestmodel_wfc3, y0_bestmodel_wfc3,
			pix_bestmodel_acs, pix_bestmodel_wfc3;

	double datafluxinner_filter1, datafluxinner_filter2, datafluxouter_filter1,
			datafluxouter_filter2, dataerrinner_filter1, dataerrinner_filter2,
			dataerrouter_filter1, dataerrouter_filter2;
	double datafluxinner_filter3, datafluxinner_filter4, datafluxouter_filter3,
			datafluxouter_filter4, dataerrinner_filter3, dataerrinner_filter4,
			dataerrouter_filter3, dataerrouter_filter4;
	double datafluxinner_filter5, datafluxinner_filter6, datafluxouter_filter5,
			datafluxouter_filter6, dataerrinner_filter5, dataerrinner_filter6,
			dataerrouter_filter5, dataerrouter_filter6;
	double datafluxinner_filter7, datafluxouter_filter7, dataerrinner_filter7,
			dataerrouter_filter7;
	double maginner_data_filter1, maginner_data_filter2, magouter_data_filter1,
			magouter_data_filter2, magerrinner_data_filter1,
			magerrinner_data_filter2, magerrouter_data_filter1,
			magerrouter_data_filter2, maginner_data_filter3,
			maginner_data_filter4, magouter_data_filter3, magouter_data_filter4,
			magerrinner_data_filter3, magerrinner_data_filter4,
			magerrouter_data_filter3, magerrouter_data_filter4,
			CG_aperture_data, CGerr_aperture_data, CG_aperture_data_err;
	double maginner_data_filter5, maginner_data_filter6, magouter_data_filter5,
			magouter_data_filter6, magerrinner_data_filter5,
			magerrinner_data_filter6, magerrouter_data_filter5,
			magerrouter_data_filter6, maginner_data_filter7,
			magouter_data_filter7, magerrinner_data_filter7,
			magerrouter_data_filter7;

	double modelfluxinner_filter1, modelfluxinner_filter2,
			modelfluxouter_filter1, modelfluxouter_filter2,
			modelfluxinner_filter3, modelfluxinner_filter4,
			modelfluxouter_filter3, modelfluxouter_filter4;
	double maginner_model_filter1, maginner_model_filter2,
			magouter_model_filter1, magouter_model_filter2,
			maginner_model_filter3, maginner_model_filter4,
			magouter_model_filter3, magouter_model_filter4, CG_aperture_model;
	double modelfluxinner_filter5, modelfluxinner_filter6,
			modelfluxouter_filter5, modelfluxouter_filter6;
	...
	double *psfpix_filter1, *psfpix_filter2, *psfpix_filter3, *psfpix_filter4,
			*psfpix_filter5, *psfpix_filter6, *psfpix_filter7;
	double xdummy, ydummy, zdummy, Idummy_filter1, norm_disk_filter1,
			norm_disk_filter2, norm_bulge_filter1, norm_bulge_filter2,
			Idummy_filter3, norm_disk_filter3, norm_disk_filter4,
			norm_disk_filter5, norm_disk_filter6, norm_disk_filter7,
			norm_bulge_filter3, norm_bulge_filter4, norm_bulge_filter5,
			norm_bulge_filter6, norm_bulge_filter7;
	int i = 0, j = 0, comp, ftsize, ig, testnumber;
	double xcentre, ycentre, scaling, theta_deg, theta_rad, posangle_rad,
			posangle_deg, scaling_best, e1_best, e2_best, theta_deg_best,
			theta_rad_best, posangle_rad_best, posangle_deg_best,
			mass_disk_chosen, mass_disk, mass_disk_best, mass_bulge_chosen, e1,
			e2, mass_bulge, mass_bulge_best, psftot1, psftot2, psftot3, psftot4,
			psftot5, psftot6, psftot7, exposuretime_filter1,
			exposuretime_filter2, exposuretime_filter3, exposuretime_filter4,
			xcentre_best, ycentre_best;
	double xcentre_step, scaling_step, e1_step, e2_step, ycentre_step;
	double xcentre_step_best, scaling_step_best, e1_step_best, e2_step_best,
			ycentre_step_best;
	double chi2_best_type, scaling_best_global, e1_best_global, e2_best_global,
			xcentre_best_global, ycentre_best_global, theta_best_global,
			posangle_best_global;
	double chi2_best_global, chi2_masked;
	int itype_best_global;
	double angle, fwhm, psfe1, psfe2, snorm, shear;
	double *wpsf, *sumpsf;
	double rvals1[Nparams], rvals2[Nparams], rvals3[Nparams], rvals4[Nparams],
			rvals5[Nparams];
	double Ro;
	double x_scaled, y_scaled, z_scaled, x_inclined, y_inclined;
	int k = 0, ncols, nrows, pix, i_index, j_index, ii, jj;
	int nrows_tot_acs, ncols_tot_acs, nrows_tot_wfc3, ncols_tot_wfc3, ii_o,
			jj_o, pix_o, pix_tot, jj_o_psf, ii_o_psf, pix_o_psf;
	double xmin, xmax, ymin, ymax;
	double disk_flux_tot_filter1 = 0.0, disk_flux_tot_filter2 = 0.0,
			disk_flux_tot_filter3 = 0.0, disk_flux_tot_filter4 = 0.0,
			disk_flux_tot_filter5 = 0.0, disk_flux_tot_filter6 = 0.0,
			disk_flux_tot_filter7 = 0.0;

	double *model_conv_disk_filter1, *model_conv_bulge_filter1,
			*model_conv_summed_filter1;
	double *model_conv_disk_filter2, *model_conv_bulge_filter2,
			*model_conv_summed_filter2;
	double *model_conv_disk_filter3, *model_conv_bulge_filter3,
			*model_conv_summed_filter3;
	double *model_conv_disk_filter4, *model_conv_bulge_filter4,
			*model_conv_summed_filter4;
	double *model_conv_disk_filter5, *model_conv_bulge_filter5,
			*model_conv_summed_filter5;
	double *model_conv_disk_filter6, *model_conv_bulge_filter6,
			*model_conv_summed_filter6;
	double *model_conv_disk_filter7, *model_conv_bulge_filter7,
			*model_conv_summed_filter7;

	double *model_conv_residual_filter1, *model_conv_residual_filter2,
			*model_conv_residual_filter3, *model_conv_residual_filter4,
			*model_conv_residual_filter5, *model_conv_residual_filter6,
			*model_conv_residual_filter7;
	double *model_unconv_disk_filter1, *model_unconv_bulge_filter1,
			*model_unconv_summed_filter1;
	double *model_unconv_disk_filter2, *model_unconv_bulge_filter2,
			*model_unconv_summed_filter2;
	double *model_unconv_disk_filter3, *model_unconv_bulge_filter3,
			*model_unconv_summed_filter3;
	double *model_unconv_disk_filter4, *model_unconv_bulge_filter4,
			*model_unconv_summed_filter4;
	double *model_unconv_disk_filter5, *model_unconv_bulge_filter5,
			*model_unconv_summed_filter5;
	double *model_unconv_disk_filter6, *model_unconv_bulge_filter6,
			*model_unconv_summed_filter6;
	double *model_unconv_disk_filter7, *model_unconv_bulge_filter7,
			*model_unconv_summed_filter7;

	double mag_bulge_filter1, mag_bulge_filter2, mag_disk_filter1,
			mag_disk_filter2, mag_bulge_filter3, mag_bulge_filter4,
			mag_bulge_filter5, mag_bulge_filter6, mag_bulge_filter7,
			mag_disk_filter3, mag_disk_filter4, mag_disk_filter5,
			mag_disk_filter6, mag_disk_filter7;
	double colour_bulge, colour_disk, CG_bulge_disk;
	double x_disk_dummy, y_disk_dummy, z_disk_dummy, I_xyz_disk_filter1_dummy,
			dummy5_disk, dummy5_bulge;
	double x_bulge_dummy, y_bulge_dummy, z_bulge_dummy,
			I_xyz_bulge_filter1_dummy;
	...
	double totflux_filter1, totflux_filter2, totflux_filter3, totflux_filter4,
			totflux_filter5, totflux_filter6, totflux_filter7;
	double sumfluxsquared_filter1 = 0.0;
	double sumfluxsquared_filter2 = 0.0;
	double sumfluxsquared_filter3 = 0.0;
	double sumfluxsquared_filter4 = 0.0, sumfluxsquared_filter5 = 0.0,
			sumfluxsquared_filter6 = 0.0, sumfluxsquared_filter7 = 0.0;
	...
	Nfreedom = (nfilters_acs * Npix_side_acs * Npix_side_acs)
			+ (nfilters_wfc3 * Npix_side_wfc3 * Npix_side_wfc3) - Nparams;
	...
	cum_model_flux_radius_filter1 = (double *) malloc(
			(nsteps) * sizeof(double));
	cum_model_flux_radius_filter2 = (double *) malloc(
			(nsteps) * sizeof(double));
	...
	cum_model_flux_radius_filter3 = (double *) malloc(
			(nsteps) * sizeof(double));
	cum_model_flux_radius_filter4 = (double *) malloc(
			(nsteps) * sizeof(double));
	...
	model_unconv_acs = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_wfc3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_acs = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_wfc3 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	psf_image_acs = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	psf_image_wfc3 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	model_conv_disk_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_bulge_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_disk_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_bulge_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_disk_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_bulge_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_disk_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_bulge_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_disk_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_conv_bulge_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_conv_disk_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_conv_bulge_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_conv_disk_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_conv_bulge_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	model_conv_summed_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_summed_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_summed_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_summed_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_summed_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_conv_summed_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_conv_summed_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	model_conv_residual_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_residual_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_residual_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_residual_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_conv_residual_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_conv_residual_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_conv_residual_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	model_unconv_disk_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_bulge_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_disk_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_bulge_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_disk_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_bulge_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_disk_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_bulge_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_disk_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_unconv_bulge_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_unconv_disk_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_unconv_bulge_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_unconv_disk_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_unconv_bulge_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	model_unconv_summed_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_summed_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_summed_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_summed_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	model_unconv_summed_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_unconv_summed_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	model_unconv_summed_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	...
	psf_image_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	psf_image_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	psf_image_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	psf_image_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	psf_image_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	psf_image_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	psf_image_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	...
	in_acs = (double*) fftw_malloc(
			sizeof(double) * Npix_side_acs * Npix_side_acs);
	identity_acs = (double*) fftw_malloc(
			sizeof(double) * Npix_side_acs * Npix_side_acs);
	final_acs = (double*) fftw_malloc(
			sizeof(double) * Npix_side_acs * Npix_side_acs);
	in_wfc3 = (double*) fftw_malloc(
			sizeof(double) * Npix_side_wfc3 * Npix_side_wfc3);
	identity_wfc3 = (double*) fftw_malloc(
			sizeof(double) * Npix_side_wfc3 * Npix_side_wfc3);
	final_wfc3 = (double*) fftw_malloc(
			sizeof(double) * Npix_side_wfc3 * Npix_side_wfc3);

	inTrans_acs = (fftw_complex*) fftw_malloc(
			sizeof(fftw_complex) * Npix_side_acs * (Npix_side_acs / 2 + 1));
	identityTrans_acs = (fftw_complex*) fftw_malloc(
			sizeof(fftw_complex) * Npix_side_acs * (Npix_side_acs / 2 + 1));
	FinalFFT_acs = (fftw_complex*) fftw_malloc(
			sizeof(fftw_complex) * Npix_side_acs * (Npix_side_acs / 2 + 1));
	inTrans_wfc3 = (fftw_complex*) fftw_malloc(
			sizeof(fftw_complex) * Npix_side_wfc3 * (Npix_side_wfc3 / 2 + 1));
	identityTrans_wfc3 = (fftw_complex*) fftw_malloc(
			sizeof(fftw_complex) * Npix_side_wfc3 * (Npix_side_wfc3 / 2 + 1));
	FinalFFT_wfc3 = (fftw_complex*) fftw_malloc(
			sizeof(fftw_complex) * Npix_side_wfc3 * (Npix_side_wfc3 / 2 + 1));

	//Initialize forward plans
	plan1_acs = fftw_plan_dft_r2c_2d(Npix_side_acs, Npix_side_acs, in_acs,
			inTrans_acs, FFTW_ESTIMATE);
	plan2_acs = fftw_plan_dft_r2c_2d(Npix_side_acs, Npix_side_acs, identity_acs,
			identityTrans_acs, FFTW_ESTIMATE);
	plan3_acs = fftw_plan_dft_c2r_2d(Npix_side_acs, Npix_side_acs, FinalFFT_acs,
			final_acs, FFTW_ESTIMATE);
	plan1_wfc3 = fftw_plan_dft_r2c_2d(Npix_side_wfc3, Npix_side_wfc3, in_wfc3,
			inTrans_wfc3, FFTW_ESTIMATE);
	plan2_wfc3 = fftw_plan_dft_r2c_2d(Npix_side_wfc3, Npix_side_wfc3,
			identity_wfc3, identityTrans_wfc3, FFTW_ESTIMATE);
	plan3_wfc3 = fftw_plan_dft_c2r_2d(Npix_side_wfc3, Npix_side_wfc3,
			FinalFFT_wfc3, final_wfc3, FFTW_ESTIMATE);

	/* Local variables x,y,z for each type */

	double ** x_bulge_type, **y_bulge_type, **z_bulge_type,
			**I_xyz_bulge_filter1_type;
	double ** x_disk_type, **y_disk_type, **z_disk_type,
			**I_xyz_disk_filter1_type;
	int iii = 0;
	...
	I_xyz_bulge_filter1_type = (double**) malloc(
			sizeof(double*) * npoints_bulge);
	for (iii = 0; iii < npoints_bulge; iii++)
		I_xyz_bulge_filter1_type[iii] = (double*) malloc(
				sizeof(double) * ntypes);
	...
	for (iii = 0; iii < npoints_disk; iii++)
		I_xyz_disk_filter1_type[iii] = (double*) malloc(
				sizeof(double) * ntypes);
	...
	for (itype = 0; itype < ntypes; itype++)
	{
		...
		if (itype == 1)
		{
			strcpy(galaxytype, "s0");
		}
		if (itype == 2)
		{
			strcpy(galaxytype, "sa");
		}
		if (itype == 3)
		{
			strcpy(galaxytype, "sb");
		}
		if (itype == 4)
		{
			strcpy(galaxytype, "sc");
		}
		if (itype == 5)
		{
			strcpy(galaxytype, "sd");
		}
		...
		kk = 0;
		while (fscanf(fp1, "%lf %lf %lf %lf ", &x_disk_dummy, &y_disk_dummy,
				&z_disk_dummy, &I_xyz_disk_filter1_dummy) == 4)
		{
		...
		ll = 0;
		while (fscanf(fp2, "%lf %lf %lf %lf ", &x_bulge_dummy, &y_bulge_dummy,
				&z_bulge_dummy, &I_xyz_bulge_filter1_dummy) == 4)
		{
	...
	for (jj = 0; jj < nrows_tot_acs; jj++)
		for (ii = 0; ii < ncols_tot_acs; ii++)
		{
			...
			if ((ii >= (ncols_tot_acs - ncols_psf_acs) / 2)
					&& (ii < (ncols_tot_acs + ncols_psf_acs) / 2)
					&& (jj >= (nrows_tot_acs - nrows_psf_acs) / 2)
					&& (jj < (nrows_tot_acs + nrows_psf_acs) / 2))
			{
	...
	for (jj = 0; jj < nrows_tot_wfc3; jj++)
		for (ii = 0; ii < ncols_tot_wfc3; ii++)
		{
			...
			if ((ii >= (ncols_tot_wfc3 - ncols_psf_wfc3) / 2)
					&& (ii < (ncols_tot_wfc3 + ncols_psf_wfc3) / 2)
					&& (jj >= (nrows_tot_wfc3 - nrows_psf_wfc3) / 2)
					&& (jj < (nrows_tot_wfc3 + nrows_psf_wfc3) / 2))
			{
	...
	/*---------------------------- */
	double *countrate_filter1, *countrate_filter2, *countrate_filter3,
			*countrate_filter4, *countrate_filter5, *countrate_filter6,
			*countrate_filter7, *wht_filter1, *wht_filter2, *wht_filter3,
			*wht_filter4, *wht_filter5, *wht_filter6, *wht_filter7;
	countrate_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	countrate_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	countrate_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	countrate_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	countrate_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	countrate_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	countrate_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	wht_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	wht_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	wht_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	wht_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	wht_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	wht_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	wht_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	mask_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	mask_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	mask_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	mask_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	mask_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	mask_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	mask_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	simdata_orig_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simdata_orig_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simdata_orig_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simdata_orig_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simdata_orig_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	simdata_orig_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	simdata_orig_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	simdata_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simrms_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simnoise_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	weight_filter1 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));

	simdata_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simrms_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simnoise_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	weight_filter2 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));

	simdata_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simrms_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simnoise_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	weight_filter3 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));

	simdata_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simrms_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	simnoise_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));
	weight_filter4 = (double *) malloc(
			(Npix_side_acs * Npix_side_acs) * sizeof(double));

	simdata_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	simrms_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	simnoise_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	weight_filter5 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	simdata_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	simrms_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	simnoise_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	weight_filter6 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));

	simdata_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	simrms_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	simnoise_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	weight_filter7 = (double *) malloc(
			(Npix_side_wfc3 * Npix_side_wfc3) * sizeof(double));
	...
	while (fscanf(fpcat,
			...
			&objid, &ra, &dec, &zphot, &R50, &VAB, &IAB, &M_I,
			&exposuretime_filter1, &exposuretime_filter2, &exposuretime_filter3,
			&exposuretime_filter4, &arcsecstokpc, &dL, &age_universe) == 15)

	{
		...
		strcpy(datafitsname_filter5,
				"/users/welikala/data/GOODS/WFC3/f105w/Ystamp");
		strcpy(whtfitsname_filter5,
				"/users/welikala/data/GOODS/WFC3/f105w/Ystampwht");
		strcpy(datafitsname_filter6,
				"/users/welikala/data/GOODS/WFC3/f125w/Jstamp");
		strcpy(whtfitsname_filter6,
				"/users/welikala/data/GOODS/WFC3/f125w/Jstampwht");
		strcpy(datafitsname_filter7,
				"/users/welikala/data/GOODS/WFC3/f160w/Hstamp");
		strcpy(whtfitsname_filter7,
				"/users/welikala/data/GOODS/WFC3/f160w/Hstampwht");
		...
		for (pix = 0; pix < npixels_acs; pix++)
		{
			simdata_filter1[pix] = countrate_filter1[pix]
					* pow(10, (48.6 + zeropoint_filter1) / (-2.5)) * 1e23 * 1e9;
			simdata_filter2[pix] = countrate_filter2[pix]
					* pow(10, (48.6 + zeropoint_filter2) / (-2.5)) * 1e23 * 1e9;
			simdata_filter3[pix] = countrate_filter3[pix]
					* pow(10, (48.6 + zeropoint_filter3) / (-2.5)) * 1e23 * 1e9;
			simdata_filter4[pix] = countrate_filter4[pix]
					* pow(10, (48.6 + zeropoint_filter4) / (-2.5)) * 1e23 * 1e9;
			simrms_filter1[pix] = sqrt(
					(pow(corr_correlations_filter1, 2) / wht_filter1[pix])
							+ (countrate_filter1[pix] / exposuretime_filter1))
					* pow(10, (48.6 + zeropoint_filter1) / (-2.5)) * 1e23 * 1e9;
			simrms_filter2[pix] = sqrt(
					(pow(corr_correlations_filter2, 2) / wht_filter2[pix])
							+ (countrate_filter2[pix] / exposuretime_filter2))
					* pow(10, (48.6 + zeropoint_filter2) / (-2.5)) * 1e23 * 1e9;
			simrms_filter3[pix] = sqrt(
					(pow(corr_correlations_filter3, 2) / wht_filter3[pix])
							+ (countrate_filter3[pix] / exposuretime_filter3))
					* pow(10, (48.6 + zeropoint_filter3) / (-2.5)) * 1e23 * 1e9;
			simrms_filter4[pix] = sqrt(
					(pow(corr_correlations_filter4, 2) / wht_filter4[pix])
							+ (countrate_filter4[pix] / exposuretime_filter4))
					* pow(10, (48.6 + zeropoint_filter4) / (-2.5)) * 1e23 * 1e9;
		}

		for (pix = 0; pix < npixels_wfc3; pix++)
		{
			simdata_filter5[pix] = countrate_filter5[pix]
					* pow(10, (48.6 + zeropoint_filter5) / (-2.5)) * 1e23 * 1e9;
			simdata_filter6[pix] = countrate_filter6[pix]
					* pow(10, (48.6 + zeropoint_filter6) / (-2.5)) * 1e23 * 1e9;
			simdata_filter7[pix] = countrate_filter7[pix]
					* pow(10, (48.6 + zeropoint_filter7) / (-2.5)) * 1e23 * 1e9;
			simrms_filter5[pix] = sqrt(
					(pow(corr_correlations_filter5, 2) / wht_filter5[pix])
							+ (countrate_filter5[pix] / exposuretime_filter5))
					* pow(10, (48.6 + zeropoint_filter5) / (-2.5)) * 1e23 * 1e9;
			simrms_filter6[pix] = sqrt(
					(pow(corr_correlations_filter6, 2) / wht_filter6[pix])
							+ (countrate_filter6[pix] / exposuretime_filter6))
					* pow(10, (48.6 + zeropoint_filter6) / (-2.5)) * 1e23 * 1e9;
			simrms_filter7[pix] = sqrt(
					(pow(corr_correlations_filter7, 2) / wht_filter7[pix])
							+ (countrate_filter7[pix] / exposuretime_filter7))
					* pow(10, (48.6 + zeropoint_filter7) / (-2.5)) * 1e23 * 1e9;
		...
		for (itype = 0; itype < ntypes; itype++)
		{
			...
			for (ipoints = 0; ipoints < npoints_disk; ipoints++)
			{
				...
				I_xyz_disk_filter1[ipoints] =
						I_xyz_disk_filter1_type[ipoints][itype];
			}
			...
			for (ipoints = 0; ipoints < npoints_bulge; ipoints++)
			{
				...
				I_xyz_bulge_filter1[ipoints] =
						I_xyz_bulge_filter1_type[ipoints][itype];
			}
			...
			chi2_best_type = fitjointmodel(scaling, e1, e2, xcentre, ycentre,
					scaling_step, e1_step, e2_step, xcentre_step, ycentre_step,
					rvals1);
			...
			chi2_best_type = fitjointmodel(scaling_best, e1_best, e2_best,
					xcentre_best, ycentre_best, scaling_step_best, e1_step_best,
					e2_step_best, xcentre_step_best, ycentre_step_best, rvals2);
		...
		x0_bestmodel_acs = (int) ((xcentre_best_global - xmin)
				/ pixelsize_kpc_acs); // ACS pixel in x direction
		y0_bestmodel_acs = (int) ((ycentre_best_global - ymin)
				/ pixelsize_kpc_acs);  // ACS pixel in y direction
		pix_bestmodel_acs = (ncols_tot_acs * y0_bestmodel_acs)
				+ x0_bestmodel_acs;
		...
		x0_bestmodel_wfc3 = (int) ((xcentre_best_global - xmin)
				/ pixelsize_kpc_wfc3); // WFC3 pixel in x direction
		y0_bestmodel_wfc3 = (int) ((ycentre_best_global - ymin)
				/ pixelsize_kpc_wfc3);  // WFC3 pixel in y direction
		pix_bestmodel_wfc3 = (ncols_tot_wfc3 * y0_bestmodel_wfc3)
				+ x0_bestmodel_wfc3;
		...
		for (ipoints = 0; ipoints < npoints_disk; ipoints++)
		{
			...
			I_xyz_disk_filter1[ipoints] =
					I_xyz_disk_filter1_type[ipoints][itype_best_global];
		}
		...
		for (ipoints = 0; ipoints < npoints_bulge; ipoints++)
		{
			...
			I_xyz_bulge_filter1[ipoints] =
					I_xyz_bulge_filter1_type[ipoints][itype_best_global];
		}

		/* ACS bands */

		/* filter 1*/
		make2Dgalaxymodelunconvolved_acs(0, 0, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_unconv_bulge_filter1[pix] = model_unconv_acs[pix];

		make2Dgalaxymodelunconvolved_acs(1, 0, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_unconv_disk_filter1[pix] = model_unconv_acs[pix];

		make2Dgalaxymodelconvolved_acs(0, 0, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_conv_bulge_filter1[pix] = model_conv_acs[pix];

		make2Dgalaxymodelconvolved_acs(1, 0, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
		...
		/* filter 2 */
		make2Dgalaxymodelunconvolved_acs(0, 1, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_unconv_bulge_filter2[pix] = model_unconv_acs[pix];

		make2Dgalaxymodelunconvolved_acs(1, 1, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_unconv_disk_filter2[pix] = model_unconv_acs[pix];

		make2Dgalaxymodelconvolved_acs(0, 1, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_conv_bulge_filter2[pix] = model_conv_acs[pix];

		make2Dgalaxymodelconvolved_acs(1, 1, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
		...
		make2Dgalaxymodelunconvolved_acs(0, 2, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_unconv_bulge_filter3[pix] = model_unconv_acs[pix];

		make2Dgalaxymodelunconvolved_acs(1, 2, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_unconv_disk_filter3[pix] = model_unconv_acs[pix];

		make2Dgalaxymodelconvolved_acs(0, 2, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_conv_bulge_filter3[pix] = model_conv_acs[pix];

		make2Dgalaxymodelconvolved_acs(1, 2, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
		...
		make2Dgalaxymodelunconvolved_acs(0, 3, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_unconv_bulge_filter4[pix] = model_unconv_acs[pix];

		make2Dgalaxymodelunconvolved_acs(1, 3, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_unconv_disk_filter4[pix] = model_unconv_acs[pix];

		make2Dgalaxymodelconvolved_acs(0, 3, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
			model_conv_bulge_filter4[pix] = model_conv_acs[pix];

		make2Dgalaxymodelconvolved_acs(1, 3, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_acs; pix++)
		...
		make2Dgalaxymodelunconvolved_wfc3(0, 4, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
			model_unconv_bulge_filter5[pix] = model_unconv_wfc3[pix];

		make2Dgalaxymodelunconvolved_wfc3(1, 4, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
			model_unconv_disk_filter5[pix] = model_unconv_wfc3[pix];

		make2Dgalaxymodelconvolved_wfc3(0, 4, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
			model_conv_bulge_filter5[pix] = model_conv_wfc3[pix];

		make2Dgalaxymodelconvolved_wfc3(1, 4, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
		...
		make2Dgalaxymodelunconvolved_wfc3(0, 5, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
			model_unconv_bulge_filter6[pix] = model_unconv_wfc3[pix];

		make2Dgalaxymodelunconvolved_wfc3(1, 5, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
			model_unconv_disk_filter6[pix] = model_unconv_wfc3[pix];

		make2Dgalaxymodelconvolved_wfc3(0, 5, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
			model_conv_bulge_filter6[pix] = model_conv_wfc3[pix];

		make2Dgalaxymodelconvolved_wfc3(1, 5, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
		...
		make2Dgalaxymodelunconvolved_wfc3(0, 6, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
			model_unconv_bulge_filter7[pix] = model_unconv_wfc3[pix];

		make2Dgalaxymodelunconvolved_wfc3(1, 6, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
			model_unconv_disk_filter7[pix] = model_unconv_wfc3[pix];

		make2Dgalaxymodelconvolved_wfc3(0, 6, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
			model_conv_bulge_filter7[pix] = model_conv_wfc3[pix];

		make2Dgalaxymodelconvolved_wfc3(1, 6, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, arcsecstokpc, 1.0);
		for (pix = 0; pix < npixels_wfc3; pix++)
		...
		for (j = 0; j < npixels_acs; j++)
		{
			model_disk_filter1[j] = model_conv_disk_filter1[j]
					/ simrms_filter1[j];
			model_bulge_filter1[j] = model_conv_bulge_filter1[j]
					/ simrms_filter1[j];
			model_disk_filter2[j] = model_conv_disk_filter2[j]
					/ simrms_filter2[j];
			model_bulge_filter2[j] = model_conv_bulge_filter2[j]
					/ simrms_filter2[j];
			model_disk_filter3[j] = model_conv_disk_filter3[j]
					/ simrms_filter3[j];
			model_bulge_filter3[j] = model_conv_bulge_filter3[j]
					/ simrms_filter3[j];
			model_disk_filter4[j] = model_conv_disk_filter4[j]
					/ simrms_filter4[j];
			model_bulge_filter4[j] = model_conv_bulge_filter4[j]
					/ simrms_filter4[j];
		...
		for (j = 0; j < npixels_wfc3; j++)
		{
			...
			model_disk_filter5[j] = model_conv_disk_filter5[j]
					/ simrms_filter5[j];
			model_bulge_filter5[j] = model_conv_bulge_filter5[j]
					/ simrms_filter5[j];
			model_disk_filter6[j] = model_conv_disk_filter6[j]
					/ simrms_filter6[j];
			model_bulge_filter6[j] = model_conv_bulge_filter6[j]
					/ simrms_filter6[j];
			model_disk_filter7[j] = model_conv_disk_filter7[j]
					/ simrms_filter7[j];
			model_bulge_filter7[j] = model_conv_bulge_filter7[j]
					/ simrms_filter7[j];
		...
		for (j = 0; j < npixels_acs; j++)
		{
			...
			if (itype_best_global == 0)
			{
				if (model_conv_bulge_filter1[j]
						> (model_conv_bulge_filter1[pix_bestmodel_acs]
								* sb_limit_bulge))
				{
					mask_filter1[j] = 1.0;
				}
				if (model_conv_bulge_filter2[j]
						> (model_conv_bulge_filter2[pix_bestmodel_acs]
								* sb_limit_bulge))
				{
					mask_filter2[j] = 1.0;
				}
				if (model_conv_bulge_filter3[j]
						> (model_conv_bulge_filter3[pix_bestmodel_acs]
								* sb_limit_bulge))
				{
					mask_filter3[j] = 1.0;
				}
				if (model_conv_bulge_filter4[j]
						> (model_conv_bulge_filter4[pix_bestmodel_acs]
								* sb_limit_bulge))
				{
					mask_filter4[j] = 1.0;
				}
			...
			if (itype_best_global > 0)
			{
				if (model_conv_disk_filter1[j]
						> (model_conv_disk_filter1[pix_bestmodel_acs]
								* sb_limit_disk))
				{
					mask_filter1[j] = 1.0;
				}
				if (model_conv_disk_filter2[j]
						> (model_conv_disk_filter2[pix_bestmodel_acs]
								* sb_limit_disk))
				{
					mask_filter2[j] = 1.0;
				}
				if (model_conv_disk_filter3[j]
						> (model_conv_disk_filter3[pix_bestmodel_acs]
								* sb_limit_disk))
				{
					mask_filter3[j] = 1.0;
				}
				if (model_conv_disk_filter4[j]
						> (model_conv_disk_filter4[pix_bestmodel_acs]
								* sb_limit_disk))
				{
					mask_filter4[j] = 1.0;
				}
		...
		for (j = 0; j < npixels_wfc3; j++)
		{
			...
			if (itype_best_global == 0)
			{
				if (model_conv_bulge_filter5[j]
						> (model_conv_bulge_filter5[pix_bestmodel_wfc3]
								* sb_limit_bulge))
				{
					mask_filter5[j] = 1.0;
				}
				if (model_conv_bulge_filter6[j]
						> (model_conv_bulge_filter6[pix_bestmodel_wfc3]
								* sb_limit_bulge))
				{
					mask_filter6[j] = 1.0;
				}
				if (model_conv_bulge_filter7[j]
						> (model_conv_bulge_filter7[pix_bestmodel_wfc3]
								* sb_limit_bulge))
				{
					mask_filter7[j] = 1.0;
				}
			...
			if (itype_best_global > 0)
			{
				...
				if (model_conv_disk_filter5[j]
						> (model_conv_disk_filter5[pix_bestmodel_wfc3]
								* sb_limit_disk))
				{
					mask_filter5[j] = 1.0;
				}
				if (model_conv_disk_filter6[j]
						> (model_conv_disk_filter6[pix_bestmodel_wfc3]
								* sb_limit_disk))
				{
					mask_filter6[j] = 1.0;
				}
				if (model_conv_disk_filter7[j]
						> (model_conv_disk_filter7[pix_bestmodel_wfc3]
								* sb_limit_disk))
				{
					mask_filter7[j] = 1.0;
				}
		...
		if (itype_best_global == 0)
		{
			...
			lval = lval + penalty_e1e2quad + penalty_scaling
					...
					+ penalty_amp_bulge_filter5 + penalty_amp_disk_filter5
					+ penalty_amp_bulge_filter6 + penalty_amp_disk_filter6
					+ penalty_amp_bulge_filter7 + penalty_amp_disk_filter7;
		...
		if (itype_best_global > 0)
		{
			...
			for (pix = 0; pix < npixels_acs; pix++)
			{
				...
				model_unconv_disk_filter1[pix] = amp_disk_filter1
						* model_unconv_disk_filter1[pix];
				model_unconv_bulge_filter1[pix] = amp_bulge_filter1
						* model_unconv_bulge_filter1[pix];
				model_conv_disk_filter1[pix] = amp_disk_filter1
						* model_conv_disk_filter1[pix];
				model_conv_bulge_filter1[pix] = amp_bulge_filter1
						* model_conv_bulge_filter1[pix];
				model_unconv_summed_filter1[pix] =
						(model_unconv_disk_filter1[pix])
								+ (model_unconv_bulge_filter1[pix]);
				model_conv_summed_filter1[pix] = (model_conv_disk_filter1[pix])
						+ (model_conv_bulge_filter1[pix]);

				totalflux_unconv_bulge_filter1 = totalflux_unconv_bulge_filter1
						+ (model_unconv_bulge_filter1[pix]);
				totalflux_unconv_disk_filter1 = totalflux_unconv_disk_filter1
						+ (model_unconv_disk_filter1[pix]);
				totalflux_conv_bulge_filter1 = totalflux_conv_bulge_filter1
						+ (model_conv_bulge_filter1[pix]);
				totalflux_conv_disk_filter1 = totalflux_conv_disk_filter1
						+ (model_conv_disk_filter1[pix]);

				model_unconv_disk_filter2[pix] = amp_disk_filter2
						* model_unconv_disk_filter2[pix];
				model_unconv_bulge_filter2[pix] = amp_bulge_filter2
						* model_unconv_bulge_filter2[pix];
				model_conv_disk_filter2[pix] = amp_disk_filter2
						* model_conv_disk_filter2[pix];
				model_conv_bulge_filter2[pix] = amp_bulge_filter2
						* model_conv_bulge_filter2[pix];
				model_unconv_summed_filter2[pix] =
						(model_unconv_disk_filter2[pix])
								+ (model_unconv_bulge_filter2[pix]);
				model_conv_summed_filter2[pix] = (model_conv_disk_filter2[pix])
						+ (model_conv_bulge_filter2[pix]);

				totalflux_unconv_bulge_filter2 = totalflux_unconv_bulge_filter2
						+ (model_unconv_bulge_filter2[pix]);
				totalflux_unconv_disk_filter2 = totalflux_unconv_disk_filter2
						+ (model_unconv_disk_filter2[pix]);
				totalflux_conv_bulge_filter2 = totalflux_conv_bulge_filter2
						+ (model_conv_bulge_filter2[pix]);
				totalflux_conv_disk_filter2 = totalflux_conv_disk_filter2
						+ (model_conv_disk_filter2[pix]);

				model_unconv_disk_filter3[pix] = amp_disk_filter3
						* model_unconv_disk_filter3[pix];
				model_unconv_bulge_filter3[pix] = amp_bulge_filter3
						* model_unconv_bulge_filter3[pix];
				model_conv_disk_filter3[pix] = amp_disk_filter3
						* model_conv_disk_filter3[pix];
				model_conv_bulge_filter3[pix] = amp_bulge_filter3
						* model_conv_bulge_filter3[pix];
				model_unconv_summed_filter3[pix] =
						(model_unconv_disk_filter3[pix])
								+ (model_unconv_bulge_filter3[pix]);
				model_conv_summed_filter3[pix] = (model_conv_disk_filter3[pix])
						+ (model_conv_bulge_filter3[pix]);

				totalflux_unconv_bulge_filter3 = totalflux_unconv_bulge_filter3
						+ (model_unconv_bulge_filter3[pix]);
				totalflux_unconv_disk_filter3 = totalflux_unconv_disk_filter3
						+ (model_unconv_disk_filter3[pix]);
				totalflux_conv_bulge_filter3 = totalflux_conv_bulge_filter3
						+ (model_conv_bulge_filter3[pix]);
				totalflux_conv_disk_filter3 = totalflux_conv_disk_filter3
						+ (model_conv_disk_filter3[pix]);

				model_unconv_disk_filter4[pix] = amp_disk_filter4
						* model_unconv_disk_filter4[pix];
				model_unconv_bulge_filter4[pix] = amp_bulge_filter4
						* model_unconv_bulge_filter4[pix];
				model_conv_disk_filter4[pix] = amp_disk_filter4
						* model_conv_disk_filter4[pix];
				model_conv_bulge_filter4[pix] = amp_bulge_filter4
						* model_conv_bulge_filter4[pix];
				model_unconv_summed_filter4[pix] =
						(model_unconv_disk_filter4[pix])
								+ (model_unconv_bulge_filter4[pix]);
				model_conv_summed_filter4[pix] = (model_conv_disk_filter4[pix])
						+ (model_conv_bulge_filter4[pix]);

				totalflux_unconv_bulge_filter4 = totalflux_unconv_bulge_filter4
						+ (model_unconv_bulge_filter4[pix]);
				totalflux_unconv_disk_filter4 = totalflux_unconv_disk_filter4
						+ (model_unconv_disk_filter4[pix]);
				totalflux_conv_bulge_filter4 = totalflux_conv_bulge_filter4
						+ (model_conv_bulge_filter4[pix]);
				totalflux_conv_disk_filter4 = totalflux_conv_disk_filter4
						+ (model_conv_disk_filter4[pix]);

				model_conv_residual_filter1[pix] = (simdata_filter1[pix]
						- model_conv_summed_filter1[pix]) / simrms_filter1[pix];
				model_conv_residual_filter2[pix] = (simdata_filter2[pix]
						- model_conv_summed_filter2[pix]) / simrms_filter2[pix];
				model_conv_residual_filter3[pix] = (simdata_filter3[pix]
						- model_conv_summed_filter3[pix]) / simrms_filter3[pix];
				model_conv_residual_filter4[pix] = (simdata_filter4[pix]
						- model_conv_summed_filter4[pix]) / simrms_filter4[pix];
			...
			for (pix = 0; pix < npixels_wfc3; pix++)
			{
				...
				model_unconv_disk_filter5[pix] = amp_disk_filter5
						* model_unconv_disk_filter5[pix];
				model_unconv_bulge_filter5[pix] = amp_bulge_filter5
						* model_unconv_bulge_filter5[pix];
				model_conv_disk_filter5[pix] = amp_disk_filter5
						* model_conv_disk_filter5[pix];
				model_conv_bulge_filter5[pix] = amp_bulge_filter5
						* model_conv_bulge_filter5[pix];
				model_unconv_summed_filter5[pix] =
						(model_unconv_disk_filter5[pix])
								+ (model_unconv_bulge_filter5[pix]);
				model_conv_summed_filter5[pix] = (model_conv_disk_filter5[pix])
						+ (model_conv_bulge_filter5[pix]);

				totalflux_unconv_bulge_filter5 = totalflux_unconv_bulge_filter5
						+ (model_unconv_bulge_filter5[pix]);
				totalflux_unconv_disk_filter5 = totalflux_unconv_disk_filter5
						+ (model_unconv_disk_filter5[pix]);
				totalflux_conv_bulge_filter5 = totalflux_conv_bulge_filter5
						+ (model_conv_bulge_filter5[pix]);
				totalflux_conv_disk_filter5 = totalflux_conv_disk_filter5
						+ (model_conv_disk_filter5[pix]);

				model_unconv_disk_filter6[pix] = amp_disk_filter6
						* model_unconv_disk_filter6[pix];
				model_unconv_bulge_filter6[pix] = amp_bulge_filter6
						* model_unconv_bulge_filter6[pix];
				model_conv_disk_filter6[pix] = amp_disk_filter6
						* model_conv_disk_filter6[pix];
				model_conv_bulge_filter6[pix] = amp_bulge_filter6
						* model_conv_bulge_filter6[pix];
				model_unconv_summed_filter6[pix] =
						(model_unconv_disk_filter6[pix])
								+ (model_unconv_bulge_filter6[pix]);
				model_conv_summed_filter6[pix] = (model_conv_disk_filter6[pix])
						+ (model_conv_bulge_filter6[pix]);

				totalflux_unconv_bulge_filter6 = totalflux_unconv_bulge_filter6
						+ (model_unconv_bulge_filter6[pix]);
				totalflux_unconv_disk_filter6 = totalflux_unconv_disk_filter6
						+ (model_unconv_disk_filter6[pix]);
				totalflux_conv_bulge_filter6 = totalflux_conv_bulge_filter6
						+ (model_conv_bulge_filter6[pix]);
				totalflux_conv_disk_filter6 = totalflux_conv_disk_filter6
						+ (model_conv_disk_filter6[pix]);

				model_unconv_disk_filter7[pix] = amp_disk_filter7
						* model_unconv_disk_filter7[pix];
				model_unconv_bulge_filter7[pix] = amp_bulge_filter7
						* model_unconv_bulge_filter7[pix];
				model_conv_disk_filter7[pix] = amp_disk_filter7
						* model_conv_disk_filter7[pix];
				model_conv_bulge_filter7[pix] = amp_bulge_filter7
						* model_conv_bulge_filter7[pix];
				model_unconv_summed_filter7[pix] =
						(model_unconv_disk_filter7[pix])
								+ (model_unconv_bulge_filter7[pix]);
				model_conv_summed_filter7[pix] = (model_conv_disk_filter7[pix])
						+ (model_conv_bulge_filter7[pix]);

				totalflux_unconv_bulge_filter7 = totalflux_unconv_bulge_filter7
						+ (model_unconv_bulge_filter7[pix]);
				totalflux_unconv_disk_filter7 = totalflux_unconv_disk_filter7
						+ (model_unconv_disk_filter7[pix]);
				totalflux_conv_bulge_filter7 = totalflux_conv_bulge_filter7
						+ (model_conv_bulge_filter7[pix]);
				totalflux_conv_disk_filter7 = totalflux_conv_disk_filter7
						+ (model_conv_disk_filter7[pix]);

				model_conv_residual_filter5[pix] = (simdata_filter5[pix]
						- model_conv_summed_filter5[pix]) / simrms_filter5[pix];
				model_conv_residual_filter6[pix] = (simdata_filter6[pix]
						- model_conv_summed_filter6[pix]) / simrms_filter6[pix];
				model_conv_residual_filter7[pix] = (simdata_filter7[pix]
						- model_conv_summed_filter7[pix]) / simrms_filter7[pix];
		...
		if (itype_best_global == 0) /* E type - only bulge so set this to zero */
		{
			...
			for (pix = 0; pix < npixels_acs; pix++)
			{
				...
				model_unconv_bulge_filter1[pix] = amp_etype_filter1
						* model_unconv_bulge_filter1[pix];
				model_conv_bulge_filter1[pix] = amp_etype_filter1
						* model_conv_bulge_filter1[pix];
				model_unconv_summed_filter1[pix] =
						model_unconv_bulge_filter1[pix];
				...
				model_unconv_bulge_filter2[pix] = amp_etype_filter2
						* model_unconv_bulge_filter2[pix];
				model_conv_bulge_filter2[pix] = amp_etype_filter2
						* model_conv_bulge_filter2[pix];
				model_unconv_summed_filter2[pix] =
						model_unconv_bulge_filter2[pix];
				...
				model_unconv_bulge_filter3[pix] = amp_etype_filter3
						* model_unconv_bulge_filter3[pix];
				model_conv_bulge_filter3[pix] = amp_etype_filter3
						* model_conv_bulge_filter3[pix];
				model_unconv_summed_filter3[pix] =
						model_unconv_bulge_filter3[pix];
				...
				model_unconv_bulge_filter4[pix] = amp_etype_filter4
						* model_unconv_bulge_filter4[pix];
				model_conv_bulge_filter4[pix] = amp_etype_filter4
						* model_conv_bulge_filter4[pix];
				model_unconv_summed_filter4[pix] =
						model_unconv_bulge_filter4[pix];
			...
			for (pix = 0; pix < npixels_wfc3; pix++)
			{
				...
				model_unconv_bulge_filter5[pix] = amp_etype_filter5
						* model_unconv_bulge_filter5[pix];
				model_conv_bulge_filter5[pix] = amp_etype_filter5
						* model_conv_bulge_filter5[pix];
				model_unconv_summed_filter5[pix] =
						model_unconv_bulge_filter5[pix];
				...
				model_unconv_bulge_filter6[pix] = amp_etype_filter6
						* model_unconv_bulge_filter6[pix];
				model_conv_bulge_filter6[pix] = amp_etype_filter6
						* model_conv_bulge_filter6[pix];
				model_unconv_summed_filter6[pix] =
						model_unconv_bulge_filter6[pix];
				...
				model_unconv_bulge_filter7[pix] = amp_etype_filter7
						* model_unconv_bulge_filter7[pix];
				model_conv_bulge_filter7[pix] = amp_etype_filter7
						* model_conv_bulge_filter7[pix];
				model_unconv_summed_filter7[pix] =
						model_unconv_bulge_filter7[pix];
		...
		for (tt = 0; tt < ncols_tot_acs; tt++)
		{
			for (ss = 0; ss < nrows_tot_acs; ss++)
			{
				...
				radius_pix = sqrt(
						pow(tt - x0_bestmodel_acs, 2)
								+ pow(ss - y0_bestmodel_acs,
										2)) * pixelsize_arcsec_acs;  ///arcsec

				weight_filter1[pix] = model_conv_summed_filter1[pix]
						/ pow(simrms_filter1[pix], 2);
				weight_filter2[pix] = model_conv_summed_filter2[pix]
						/ pow(simrms_filter2[pix], 2);
				weight_filter3[pix] = model_conv_summed_filter3[pix]
						/ pow(simrms_filter3[pix], 2);
				weight_filter4[pix] = model_conv_summed_filter4[pix]
						/ pow(simrms_filter4[pix], 2);
		...
		for (tt = 0; tt < ncols_tot_wfc3; tt++)
		{
			for (ss = 0; ss < nrows_tot_wfc3; ss++)
			{
				...
				radius_pix = sqrt(
						pow(tt - x0_bestmodel_wfc3, 2)
								+ pow(ss - y0_bestmodel_wfc3,
										2)) * pixelsize_arcsec_wfc3;  ///arcsec

				weight_filter5[pix] = model_conv_summed_filter5[pix]
						/ pow(simrms_filter5[pix], 2);
				weight_filter6[pix] = model_conv_summed_filter6[pix]
						/ pow(simrms_filter6[pix], 2);
				weight_filter7[pix] = model_conv_summed_filter7[pix]
						/ pow(simrms_filter7[pix], 2);
		...
		for (tt = 0; tt < ncols_tot_acs; tt++)
		{
			for (ss = 0; ss < nrows_tot_acs; ss++)
			{
				...
				if (radius_pix <= (0.3 * R50))
				{
					...
					sumflux_data_inside_filter1 += (weight_filter1[pix]
							* simdata_filter1[pix]);
					sumflux_data_inside_filter2 += (weight_filter2[pix]
							* simdata_filter2[pix]);
					sumflux_data_inside_filter3 += (weight_filter3[pix]
							* simdata_filter3[pix]);
					sumflux_data_inside_filter4 += (weight_filter4[pix]
							* simdata_filter4[pix]);

					sumflux_model_inside_filter1 +=
							(model_conv_summed_filter1[pix]);
					sumflux_model_inside_filter2 +=
							(model_conv_summed_filter2[pix]);
					sumflux_model_inside_filter3 +=
							(model_conv_summed_filter3[pix]);
					sumflux_model_inside_filter4 +=
							(model_conv_summed_filter4[pix]);
				...
				if ((radius_pix > (0.3 * R50)) && (radius_pix <= R50))
				{
					...
					sumflux_data_outside_filter1 += (weight_filter1[pix]
							* simdata_filter1[pix]);
					sumflux_data_outside_filter2 += (weight_filter2[pix]
							* simdata_filter2[pix]);
					sumflux_data_outside_filter3 += (weight_filter3[pix]
							* simdata_filter3[pix]);
					sumflux_data_outside_filter4 += (weight_filter4[pix]
							* simdata_filter4[pix]);

					sumflux_model_outside_filter1 +=
							(model_conv_summed_filter1[pix]);
					sumflux_model_outside_filter2 +=
							(model_conv_summed_filter2[pix]);
					sumflux_model_outside_filter3 +=
							(model_conv_summed_filter3[pix]);
					sumflux_model_outside_filter4 +=
							(model_conv_summed_filter4[pix]);
				...
				sumflux_data_galaxy_filter1 += (weight_filter1[pix]
						* simdata_filter1[pix]);
				sumflux_data_galaxy_filter2 += (weight_filter2[pix]
						* simdata_filter2[pix]);
				sumflux_data_galaxy_filter3 += (weight_filter3[pix]
						* simdata_filter3[pix]);
				sumflux_data_galaxy_filter4 += (weight_filter4[pix]
						* simdata_filter4[pix]);
				...
				sumflux_model_galaxy_filter1 +=
						(model_unconv_summed_filter1[pix]);
				sumflux_model_galaxy_filter2 +=
						(model_unconv_summed_filter2[pix]);
				sumflux_model_galaxy_filter3 +=
						(model_unconv_summed_filter3[pix]);
				sumflux_model_galaxy_filter4 +=
						(model_unconv_summed_filter4[pix]);
		...
		for (tt = 0; tt < ncols_tot_wfc3; tt++)
		{
			for (ss = 0; ss < nrows_tot_wfc3; ss++)
			{
				...
				if (radius_pix <= (0.3 * R50))
				{
					...
					sumflux_data_inside_filter5 += (weight_filter5[pix]
							* simdata_filter5[pix]);
					sumflux_data_inside_filter6 += (weight_filter6[pix]
							* simdata_filter6[pix]);
					sumflux_data_inside_filter7 += (weight_filter7[pix]
							* simdata_filter7[pix]);

					sumflux_model_inside_filter5 +=
							(model_conv_summed_filter5[pix]);
					sumflux_model_inside_filter6 +=
							(model_conv_summed_filter6[pix]);
					sumflux_model_inside_filter7 +=
							(model_conv_summed_filter7[pix]);
				...
				if ((radius_pix > (0.3 * R50)) && (radius_pix <= R50))
				{
					...
					sumflux_data_outside_filter5 += (weight_filter5[pix]
							* simdata_filter5[pix]);
					sumflux_data_outside_filter6 += (weight_filter6[pix]
							* simdata_filter6[pix]);
					sumflux_data_outside_filter7 += (weight_filter7[pix]
							* simdata_filter7[pix]);

					sumflux_model_outside_filter5 +=
							(model_conv_summed_filter5[pix]);
					sumflux_model_outside_filter6 +=
							(model_conv_summed_filter6[pix]);
					sumflux_model_outside_filter7 +=
							(model_conv_summed_filter7[pix]);
				...
				sumflux_data_galaxy_filter5 += (weight_filter5[pix]
						* simdata_filter5[pix]);
				sumflux_data_galaxy_filter6 += (weight_filter6[pix]
						* simdata_filter6[pix]);
				sumflux_data_galaxy_filter7 += (weight_filter7[pix]
						* simdata_filter7[pix]);
				...
				sumflux_model_galaxy_filter5 +=
						(model_unconv_summed_filter5[pix]);
				sumflux_model_galaxy_filter6 +=
						(model_unconv_summed_filter6[pix]);
				sumflux_model_galaxy_filter7 +=
						(model_unconv_summed_filter7[pix]);
		...
		sumflux_data_inside_filter1 = sumflux_data_inside_filter1
				/ sum_weights_inside_filter1;
		sumflux_data_inside_filter2 = sumflux_data_inside_filter2
				/ sum_weights_inside_filter2;
		sumflux_data_inside_filter3 = sumflux_data_inside_filter3
				/ sum_weights_inside_filter3;
		sumflux_data_inside_filter4 = sumflux_data_inside_filter4
				/ sum_weights_inside_filter4;
		sumflux_data_inside_filter5 = sumflux_data_inside_filter5
				/ sum_weights_inside_filter5;
		sumflux_data_inside_filter6 = sumflux_data_inside_filter6
				/ sum_weights_inside_filter6;
		sumflux_data_inside_filter7 = sumflux_data_inside_filter7
				/ sum_weights_inside_filter7;

		sumflux_data_outside_filter1 = sumflux_data_outside_filter1
				/ sum_weights_outside_filter1;
		sumflux_data_outside_filter2 = sumflux_data_outside_filter2
				/ sum_weights_outside_filter2;
		sumflux_data_outside_filter3 = sumflux_data_outside_filter3
				/ sum_weights_outside_filter3;
		sumflux_data_outside_filter4 = sumflux_data_outside_filter4
				/ sum_weights_outside_filter4;
		sumflux_data_outside_filter5 = sumflux_data_outside_filter5
				/ sum_weights_outside_filter5;
		sumflux_data_outside_filter6 = sumflux_data_outside_filter6
				/ sum_weights_outside_filter6;
		sumflux_data_outside_filter7 = sumflux_data_outside_filter7
				/ sum_weights_outside_filter7;

		sumflux_data_galaxy_filter1 = sumflux_data_galaxy_filter1
				/ sum_weights_galaxy_filter1;
		sumflux_data_galaxy_filter2 = sumflux_data_galaxy_filter2
				/ sum_weights_galaxy_filter2;
		sumflux_data_galaxy_filter3 = sumflux_data_galaxy_filter1
				/ sum_weights_galaxy_filter1;
		sumflux_data_galaxy_filter4 = sumflux_data_galaxy_filter2
				/ sum_weights_galaxy_filter2;
		sumflux_data_galaxy_filter5 = sumflux_data_galaxy_filter5
				/ sum_weights_galaxy_filter5;
		sumflux_data_galaxy_filter6 = sumflux_data_galaxy_filter6
				/ sum_weights_galaxy_filter6;
		sumflux_data_galaxy_filter7 = sumflux_data_galaxy_filter7
				/ sum_weights_galaxy_filter7;
		...
		mag_data_inside_filter1 = -2.5 * log10(sumflux_data_inside_filter1)
				+ 31.426;
		mag_data_inside_filter2 = -2.5 * log10(sumflux_data_inside_filter2)
				+ 31.426;
		mag_data_inside_filter3 = -2.5 * log10(sumflux_data_inside_filter3)
				+ 31.426;
		mag_data_inside_filter4 = -2.5 * log10(sumflux_data_inside_filter4)
				+ 31.426;
		mag_data_inside_filter5 = -2.5 * log10(sumflux_data_inside_filter5)
				+ 31.426;
		mag_data_inside_filter6 = -2.5 * log10(sumflux_data_inside_filter6)
				+ 31.426;
		mag_data_inside_filter7 = -2.5 * log10(sumflux_data_inside_filter7)
				+ 31.426;

		mag_data_outside_filter1 = -2.5 * log10(sumflux_data_outside_filter1)
				+ 31.426;
		mag_data_outside_filter2 = -2.5 * log10(sumflux_data_outside_filter2)
				+ 31.426;
		mag_data_outside_filter3 = -2.5 * log10(sumflux_data_outside_filter3)
				+ 31.426;
		mag_data_outside_filter4 = -2.5 * log10(sumflux_data_outside_filter4)
				+ 31.426;
		mag_data_outside_filter5 = -2.5 * log10(sumflux_data_outside_filter5)
				+ 31.426;
		mag_data_outside_filter6 = -2.5 * log10(sumflux_data_outside_filter6)
				+ 31.426;
		mag_data_outside_filter7 = -2.5 * log10(sumflux_data_outside_filter7)
				+ 31.426;

		mag_data_galaxy_filter1 = -2.5 * log10(sumflux_data_galaxy_filter1)
				+ 31.426;
		mag_data_galaxy_filter2 = -2.5 * log10(sumflux_data_galaxy_filter2)
				+ 31.426;
		mag_data_galaxy_filter3 = -2.5 * log10(sumflux_data_galaxy_filter3)
				+ 31.426;
		mag_data_galaxy_filter4 = -2.5 * log10(sumflux_data_galaxy_filter4)
				+ 31.426;
		mag_data_galaxy_filter5 = -2.5 * log10(sumflux_data_galaxy_filter5)
				+ 31.426;
		mag_data_galaxy_filter6 = -2.5 * log10(sumflux_data_galaxy_filter6)
				+ 31.426;
		mag_data_galaxy_filter7 = -2.5 * log10(sumflux_data_galaxy_filter7)
				+ 31.426;

		mag_model_inside_filter1 = -2.5 * log10(sumflux_model_inside_filter1)
				+ 31.426;
		mag_model_inside_filter2 = -2.5 * log10(sumflux_model_inside_filter2)
				+ 31.426;
		mag_model_inside_filter3 = -2.5 * log10(sumflux_model_inside_filter3)
				+ 31.426;
		mag_model_inside_filter4 = -2.5 * log10(sumflux_model_inside_filter4)
				+ 31.426;
		mag_model_inside_filter5 = -2.5 * log10(sumflux_model_inside_filter5)
				+ 31.426;
		mag_model_inside_filter6 = -2.5 * log10(sumflux_model_inside_filter6)
				+ 31.426;
		mag_model_inside_filter7 = -2.5 * log10(sumflux_model_inside_filter7)
				+ 31.426;

		mag_model_outside_filter1 = -2.5 * log10(sumflux_model_outside_filter1)
				+ 31.426;
		mag_model_outside_filter2 = -2.5 * log10(sumflux_model_outside_filter2)
				+ 31.426;
		mag_model_outside_filter3 = -2.5 * log10(sumflux_model_outside_filter3)
				+ 31.426;
		mag_model_outside_filter4 = -2.5 * log10(sumflux_model_outside_filter4)
				+ 31.426;
		mag_model_outside_filter5 = -2.5 * log10(sumflux_model_outside_filter5)
				+ 31.426;
		mag_model_outside_filter6 = -2.5 * log10(sumflux_model_outside_filter6)
				+ 31.426;
		mag_model_outside_filter7 = -2.5 * log10(sumflux_model_outside_filter7)
				+ 31.426;

		mag_model_galaxy_filter1 = -2.5 * log10(sumflux_model_galaxy_filter1)
				+ 31.426;
		mag_model_galaxy_filter2 = -2.5 * log10(sumflux_model_galaxy_filter2)
				+ 31.426;
		mag_model_galaxy_filter3 = -2.5 * log10(sumflux_model_galaxy_filter3)
				+ 31.426;
		mag_model_galaxy_filter4 = -2.5 * log10(sumflux_model_galaxy_filter4)
				+ 31.426;
		mag_model_galaxy_filter5 = -2.5 * log10(sumflux_model_galaxy_filter5)
				+ 31.426;
		mag_model_galaxy_filter6 = -2.5 * log10(sumflux_model_galaxy_filter6)
				+ 31.426;
		mag_model_galaxy_filter7 = -2.5 * log10(sumflux_model_galaxy_filter7)
				+ 31.426;
		...
		btratio_filter1 = amp_bulge_filter1
				/ (amp_bulge_filter1 + amp_disk_filter1);
		btratio_filter2 = amp_bulge_filter2
				/ (amp_bulge_filter2 + amp_disk_filter2);
		btratio_filter3 = amp_bulge_filter3
				/ (amp_bulge_filter3 + amp_disk_filter3);
		btratio_filter4 = amp_bulge_filter4
				/ (amp_bulge_filter4 + amp_disk_filter4);
		btratio_filter5 = amp_bulge_filter5
				/ (amp_bulge_filter5 + amp_disk_filter5);
		btratio_filter6 = amp_bulge_filter6
				/ (amp_bulge_filter6 + amp_disk_filter6);
		btratio_filter7 = amp_bulge_filter7
				/ (amp_bulge_filter7 + amp_disk_filter7);
		...
		fprintf(cgfile_bulge_disk,
				"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",
				objid, ra, dec, zphot, dL, age_universe, arcsecstokpc, R50, VAB,
				IAB, M_I, itype_best_global, scaling_best_global,
				e1_best_global, e2_best_global, xcentre_best_global,
				ycentre_best_global, sumflux_bulge_filter1,
				sumflux_bulge_filter2, sumflux_bulge_filter3,
				sumflux_bulge_filter4, sumflux_bulge_filter5,
				sumflux_bulge_filter6, sumflux_bulge_filter7,
				sumflux_disk_filter1, sumflux_disk_filter2,
				sumflux_disk_filter3, sumflux_disk_filter4,
				sumflux_disk_filter5, sumflux_disk_filter6,
				sumflux_disk_filter7, sumrms_data_galaxy_filter1,
				sumrms_data_galaxy_filter2, sumrms_data_galaxy_filter3,
				sumrms_data_galaxy_filter4, sumrms_data_galaxy_filter5,
				sumrms_data_galaxy_filter6, sumrms_data_galaxy_filter7,
				btratio_filter1, btratio_filter2, btratio_filter3,
				btratio_filter4, btratio_filter5, btratio_filter5,
				btratio_filter6, btratio_filter7, chi2_best_global / Nfreedom); // 47 arguments

		fprintf(cgfile_in_out,
				"%d %lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",
				objid, ra, dec, zphot, arcsecstokpc, R50, VAB, IAB, M_I,
				sumflux_data_inside_filter1, sumflux_model_inside_filter1,
				sumflux_data_outside_filter1, sumflux_model_outside_filter1,
				sumflux_data_inside_filter2, sumflux_model_inside_filter2,
				sumflux_data_outside_filter2, sumflux_model_outside_filter2,
				sumflux_data_inside_filter3, sumflux_model_inside_filter3,
				sumflux_data_outside_filter3, sumflux_model_outside_filter3,
				sumflux_data_inside_filter4, sumflux_model_inside_filter4,
				sumflux_data_outside_filter4, sumflux_model_outside_filter4,
				sumflux_data_inside_filter5, sumflux_model_inside_filter5,
				sumflux_data_outside_filter5, sumflux_model_outside_filter5,
				sumflux_data_inside_filter6, sumflux_model_inside_filter6,
				sumflux_data_outside_filter6, sumflux_model_outside_filter6,
				sumflux_data_inside_filter7, sumflux_model_inside_filter7,
				sumflux_data_outside_filter7, sumflux_model_outside_filter7,
				sumrms_data_galaxy_filter1, sumrms_data_galaxy_filter2,
				sumrms_data_galaxy_filter3, sumrms_data_galaxy_filter4,
				sumrms_data_galaxy_filter5, sumrms_data_galaxy_filter6,
				sumrms_data_galaxy_filter7);  //45 arguments

		fprintf(chi2file, "%d %10.7lf %10.7lf %lf %d %lf %lf\n", objid, ra, dec,
				zphot, itype_best_global, chi2_best_global / Nfreedom,
				chi2_masked / nfreedom_mask);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs,
				model_conv_summed_filter1, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs,
				model_conv_summed_filter2, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs,
				model_conv_summed_filter3, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs,
				model_conv_summed_filter4, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs,
				model_conv_residual_filter1, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs,
				model_conv_residual_filter2, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs,
				model_conv_residual_filter3, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs,
				model_conv_residual_filter4, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs, mask_filter1,
				&status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs, mask_filter2,
				&status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs, mask_filter3,
				&status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_acs, mask_filter4,
				&status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_wfc3,
				model_conv_summed_filter5, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_wfc3,
				model_conv_summed_filter6, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_wfc3,
				model_conv_summed_filter7, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_wfc3, mask_filter5,
				&status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_wfc3, mask_filter6,
				&status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_wfc3, mask_filter7,
				&status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_wfc3,
				model_conv_residual_filter5, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_wfc3,
				model_conv_residual_filter6, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
		...
		if (fits_write_pix(tfptr, TDOUBLE, fpixel, npixels_wfc3,
				model_conv_residual_filter7, &status))
		{
			printf(" error writing postagestamp test pixel data %s \n",
					stampname);
			fits_report_error(stderr, status);
